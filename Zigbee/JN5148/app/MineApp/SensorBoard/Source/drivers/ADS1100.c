 ...
 * NOTES:
 *  *************************************************************************/
PUBLIC uint8 ADS1100_ReadBlock (uint8 u8channel)
{
    ADS1100_SetChannel (u8channel);
    //for(i=1000;i>0;i--);
    ADS1100_Read ();
    if(!(ADS1100_Value_State.C.AD_State&0x80))
    {
        ADS1100_Start();
        /*PrintfUtil_vPrintf("AD1: %d  AD2: %d AD3: %d AD4: %d AD5: %d AD6: %d AD7: %d AD8: %d \n\r",
        		              u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [0].value),
                	      u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [1].value),
               		      u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [2].value),
                		  u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [3].value),
                		  u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [4].value),
                		  u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [5].value),
                		  u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [6].value),
                		  u16Big_To_Little(mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [7].value));*/
        switch(u8channel)
        {
             case MULTIPLEXERS_ONE :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [7].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_TWO :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [0].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
                 //PrintfUtil_vPrintf("AD1: %d \n\r", mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [0].value);
             break;
             case MULTIPLEXERS_THREE :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [1].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_FOUR :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [2].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_FIVE :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [3].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_SIX :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [4].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_SEVEN :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [5].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             case MULTIPLEXERS_EIGHT :
            	 mbus_hdr_slv.mbus_tlv.mbus_sensor_packed.mbus_sensor [6].value = u16Big_To_Little(ABS((short)ADS1100_Value_State.C.AD_Value));
             break;
             default:
             break;
        }
        u8channel++;
        if (u8channel > 7) u8channel=0;
    }
    return (u8channel);
}
