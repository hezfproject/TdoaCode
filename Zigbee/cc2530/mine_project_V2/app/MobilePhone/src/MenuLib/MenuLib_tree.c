...
static void menu_set_station_display(void);
static void menu_set_station_onkey(uint8 keys, uint8 status);
#endif
#ifdef CFG_TEST_WIRELESS
static void menu_test_wireless_display(void);
static void menu_test_wireless_onkey(uint8 keys, uint8 status);
#endif
uint8 enter_scan(uint8*p , uint8 len);
#define  INVALID_DATA   0xff

#ifdef CFG_STATION_CHECK
static void menu_find_station_signal(uint16 id, menu_station_type_t type, int8 rssi);
...
//#ifdef MENU_RF_DEBUG
    ...
    ,
    {
        ...
        .ChildNum = 0,
        .ItemName_CH = NULL,
        .oper.display = menu_set_station_display,
        .oper.on_key = menu_set_station_onkey,
    ...
    ,
    {
        ...
        .oper.on_key = menu_test_wireless_onkey,
...
static void    menu_call_record_display(void)
{
    ...
    if((pos > 0) &&(pos <= SCREEN_LINES))
    {
        ...
        for(i=pos, j=1; i>=1; i--, j++)
        {
            ...
            if(Is_inContact_SearchContactByNum_and_item(&c_node,Contect_item,record.num.nbr))
            {
                LCD_Str_Print(c_node.name, len+1, j, TRUE);
            }
            else
            {
                num_term2str(num_str,&record.num);
                LCD_Str_Print(num_str, len+1, j, TRUE);
            }
    ...
    else if((pos > SCREEN_LINES) &&(pos <= MAX_CALL_NUM))
    {
        ...
        for(i = 1, j=node_info.show_item-1; i<=3; i++)
        {
            ...
            {
                num_term2str(num_str,&record.num);
                LCD_Str_Print(num_str, len+1, i, TRUE);
            }
...
static void    menu_shortmessage_inbox_display(void)
{
    ...
    else if(node.ChildNum <= SCREEN_LINES)
    {
        ...
        for(i=node.ChildNum, j=1; i>0; i--, j++)
        {
            ...
            if(Is_inContact_SearchContactByNum_and_item(&c_node,Contect_item,sms.head.nmbr.nbr))
            {
    ...
    else
    {
        ...
        for(i=0; i<SCREEN_LINES; i++)
        {
            ...
            else
            {
                ...
                LCD_Str_Print(num_buf.p, xpos+1, i+1, TRUE);
            }
...
static void    menu_shortmessage_read_display(void)
{
    ...
    Contact_Node c_node;
    Contect_item= BUILD_UINT16(sms.head.Contect_item_L, (uint8)0x04);
    if(Is_inContact_SearchContactByNum_and_item(&c_node,Contect_item,&pterm))
    {
...
static void    menu_contact_detail_display(void)
{
    ...
    LCD_Str_Print(num_buf.p, 0, 2, TRUE);
    num_buf.len = osal_strlen((char*)num_buf.p);
    num_buf.p[num_buf.len] = '\0';
}
...
static void menu_station_check_display(void)
{
    ...
    {
        LcdClearDisplay();
        LCD_Str_Print((uint8 *)node.name, 1, 0, TRUE);//(LCD_LINE_WIDTH-osal_strlen(node.name))/2
        MENU_RESET_NODEINFO();
        if(SUCCESS!=osal_start_timerEx(MP_TaskId, MP_STATION_CHECK_EVENT, 1))
        {
            SystemReset();
        }
    }
    ...
    /* count received  id */
    uint8 station_num = 0;
    for(uint8 i=0; i<MENU_STATION_CHECK_NUM; i++)
    {
        if(menu_station_info[i].station_id > 0)
        {
            station_num++;
        }
    }
    node.ChildNum = station_num;
    Menu_Tree[GetIdxFromID(CurrentNodeID)].ChildNum = station_num;
    ...
    else if(node.ChildNum <= SCREEN_LINES)
    {
        for(i=0; i<node.ChildNum - node_info.show_item; i++)
        {
            menu_generate_station_signal_buf(menu_station_info[node_info.show_item+i].station_id,
                                             menu_station_info[node_info.show_item+i].station_type,
                                             menu_station_info[node_info.show_item+i].rssi);
            LCD_Str_Print((uint8 *)menu_station_info_buf, 0, i+1, TRUE);
        }
    }
    else
    {
...
static void menu_set_panid_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_SELECT:
    {
        ...
        if((pan_id > 0x752F && pan_id != 0xFFFF)||(pan_id < 0x4E20 ))
        {
...
static void menu_set_station_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_SELECT:
    {
        ...
        V2SimulateStationID = pan_id;

        LcdClearDisplay();
        /* show message in global buffer, and jump back with mark after 1 second */
        LCD_Str_Print(num_buf.p , (LCD_LINE_WIDTH-num_buf.len)/2 , 0, TRUE);
        LCD_Str_Print("正在发送...", 3, 1, TRUE);
        LCD_Str_Print("按CANCE退出!", 3, 2, TRUE);

        break;
    }
    case HAL_KEY_BACKSPACE:
        V2SimulateStationID = 0;
        Buffer_Clear(&num_buf);
        menu_steptoparent();
        break;
    case HAL_KEY_0:
...
static void menu_test_wireless_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_SELECT:
    {
        ...
        /* show message in global buffer, and jump back with mark after 1 second */
        LCD_Str_Print(num_buf.p , (LCD_LINE_WIDTH-num_buf.len)/2 , 0, TRUE);
        LCD_Str_Print("...CANCE退出...", 0, 1, TRUE);
        break;
    }
    case HAL_KEY_BACKSPACE:
        testDevID = 0;
...
void menu_parse_station_signal(uint16 panid, uint16 shortaddr, uint8 *pInd, uint16 len, int8 rssi)
{
    app_header_t *pHeader  = (app_header_t *)(pInd);
    ...
    if(pHeader->protocoltype == APP_PROTOCOL_TYPE_STATION ||
            pHeader->protocoltype == APP_PROTOCOL_TYPE_CARD
      )
    {
        if((shortaddr ==0) &&(pHeader->msgtype == 8)&& (*(pInd+9)&0x01))  //station
        //if(shortaddr ==0)  //station
        {
            menu_find_station_signal(panid, STATION_TYPE_STATION, rssi);
        }
        else if(panid == 0xFFF2)  //locator
        {
            menu_find_station_signal(shortaddr, STATION_TYPE_LOCATOR, rssi);
        }
    }
    else if(pHeader->protocoltype == APP_PROTOCOL_TYPE_MOBILE)
    {
        if(shortaddr ==0)  //station
        {
            menu_find_station_signal(panid, STATION_TYPE_MP_STATION, rssi);
        }
    }
}
#endif
...
#ifdef CFG_STATION_CHECK
void menu_find_station_signal(uint16 id, menu_station_type_t type, int8 rssi)
{
    for(uint8 i=0; i<MENU_STATION_CHECK_NUM; i++)
    {
        if(menu_station_info[i].station_id == id && menu_station_info[i].station_type == type)  // find old one
        {
            menu_station_info[i].rssi = rssi;
            break;
        }
        else if(menu_station_info[i].station_id == 0)  //insert new one
        {
            menu_station_info[i].station_id = id;
            menu_station_info[i].station_type = type;
            menu_station_info[i].rssi = rssi;
            break;
        }
    }
}
char*  menu_generate_station_signal_buf(uint16 id, menu_station_type_t type, int8 rssi)
{
    memset(menu_station_info_buf,0,16);

    if (type == STATION_TYPE_STATION)
    {
        sprintf(menu_station_info_buf, "主 %u %d", id, rssi);
    }
    else if(type == STATION_TYPE_LOCATOR)
    {
        sprintf(menu_station_info_buf, "辅 %u %d", id, rssi);
    }
    else if(type == STATION_TYPE_MP_STATION)
    {
        sprintf(menu_station_info_buf, "通 %u %d", id, rssi);
    }
    else if(type == STATION_TYPE_RSSI_STATION)
    {
        sprintf(menu_station_info_buf, "RSI %u %d", id, rssi);
    }
    return menu_station_info_buf;
}
#endif
