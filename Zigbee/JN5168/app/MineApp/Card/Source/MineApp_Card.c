...
PRIVATE void vInitCardSystem(void)
{
    ...
    vAHI_DioSetDirection(TOF_HELP_PIN | TOF_OK_PIN |TOF_CHECKIN_PIN, 0);
    vAHI_DioWakeEnable(TOF_HELP_PIN | TOF_OK_PIN |TOF_CHECKIN_PIN, 0);
    vAHI_DioWakeEdge(0, TOF_HELP_PIN | TOF_OK_PIN |TOF_CHECKIN_PIN);
	...
	DBG(PrintfUtil_vPrintf("light:%d\n",u8Light);)
...
//vCardCast(0xAA, 0xF000, u16SeqNum, 0, 0); // for test: record the wakeup time
    ...
    case TOF_LOCATOR_EVENT:
    {
        ...
        DBG(PrintfUtil_vPrintf("wait locator\n");)
//vCardCast(0xAA, 0xF000, u16SeqNum, 0, 0); // for test: record the wakeup time
...
PRIVATE void vBuzzer(uint32 u32Ms)
{
    ...
    vAHI_TimerStartRepeat(TOF_BUZZER_TIMER, 12, 23);
    u8ProtectCounter = 0;
}
...
PRIVATE void vBootsh_show_menu(void)
{
	...
	PrintfUtil_vPrintf("*         light=?  (light or not)     *\n");
...
PRIVATE void _card_light_process(char *option, const char *val_pos, uint32* pVal)
{
    int id;
    ...
    {
        if (id > 1 && !strcasecmp(option, OPTION_LIGHT))
        {
            PrintfUtil_vPrintf("%s\b value error\n", option);
            return;
        }

        cfg_option.u8Light = id;
        PrintfUtil_vPrintf("%s\b value ok\n", option);
    }
}
...
PRIVATE void vBootsh_run_line(char* cmd, int cmdlen)
{
    ...
    BOOTSH_CMD_T cmd_process[] = {
		...
		{OPTION_LIGHT,_card_light_process,NULL}
    };
...
PRIVATE void vGet_MACAddress(void)
{
	...
	if(u8Light > 1)
	{
		uint8 light = (psMacAddr.u32L) & 0xFF;
		if(light < 2)
		{
			u8Light = light;
		}
