/***********************************************
hal_spi.c 
    Revised:        $Date: 2010/11/10 00:10:13 $
...
#define DMA_TX(cfg )  \
st( \
  SPI_OUTPUT_CONFIG_UART0_ON_P0_SSN_OPEN(); \
...
#define SPI_OUTPUT_CONFIG_UART0_ON_P0_SSN_OPEN() \
st(  \
	P0 &= ~0x10;	\
)

#define SPI_OUTPUT_CONFIG_UART0_ON_P0_SSN_CLOSE() \
st(  \
	P0 |= 0x10;	\
)
...
void HalSpiInit(void)
{
	...
	SPI_OUTPUT_CONFIG_UART0_ON_P0_SSN_CLOSE();
	/*init signal lines*/
	SPI_SIGNAL_INIT();
...
void HalSpiStart(halSPICfg_t* config)
{
	...
	if (!pbufpool)
	{
		for (; i < POOLSIZE; ++i)
		{
			buf = osal_msg_allocate(rxConfig->bufSize + 10);  // add some size to avoid error
...
{
	uint8* txbuf = 0;
	if(len > rxConfig->bufSize)
	{
		len = rxConfig->bufSize;
	}
#ifdef CFG_SPIEVENT_CHECK
	afIncomingMSGPacket_t* p = (afIncomingMSGPacket_t *)buf;
	p->hdr.event = SPI_RX;
	p->hdr.status = 0;
#endif
...
__near_func uint8 HalSPIRxStart(void)
{
	...
	*/
	...
	rxConfig->rxBuf = osal_msg_allocate(rxConfig->bufSize + 10);  // add some size to avoid error
	if (!rxConfig->rxBuf)
...
__near_func void HalSPIPoll( void )
{
  	if (HAL_DMA_CHECK_IRQ(HAL_DMA_CH_RX)
		||(ACTIVE_LOW(P0&BV(7)) && !HAL_DMA_CHECK_ARM(HAL_DMA_CH_RX) && HalCheckRxState()))
	{
	...
	if (HAL_DMA_CHECK_IRQ(HAL_DMA_CH_TX)
		||(!HAL_DMA_CHECK_ARM(HAL_DMA_CH_TX) && HalCheckTxState()))
	{
...
void Spi_SendEvt( void* msg)
{
	...
	{
#ifdef CFG_SPIEVENT_CHECK
		afIncomingMSGPacket_t* p = (afIncomingMSGPacket_t *)msg;
		p->hdr.event = SPI_RX;
		p->hdr.status = 0;
#endif		
