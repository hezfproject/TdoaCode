...
Revised:        $Date: 2011/08/12 01:32:02 $
...
static void MP_ParseConveyor(app_Conveyor_info_t *pConveyor_s);
...
void Conveyor_Set_resent(uint8 data);
uint8 Conveyor_Get_isresent(void);
static uint8 Conver_resent;
/**************************************************************************************************
*
* @fn          MP_Init
*
...
byte MP_ParseAppFrame(const sData_t data, uint16 panid, uint16 shortAddr, int8 rssi)
{
    ...
    case CONVEYOR_INFO:
    {
        app_Conveyor_info_t*p = (app_Conveyor_info_t*)pPayload.p;

        if(pPayload.len == sizeof(app_Conveyor_info_t)
                && panid == MP_DevInfo.CoordPanID
                && shortAddr == 0
                //&& MP_termNmbrCheck(&p->nbr)
          )
...
void MP_ParseSms(app_SMS_t* pSms)
{
    if(IS_IDLE() || ON_WAKE())   //only receive sms when system is IDLE
    {
        ...
        if(count < MAX_SMS_NUM)
        {
            ...
            else  sms.head.Contect_item_L=LIST_ITEM_NULL;
...
void MP_ParseConveyor(app_Conveyor_info_t *pConveyor_s)
{

    uint8  flag;
    Conveyor_sms_t  conveyor_s;

    if(pConveyor_s->crc!=CRC16((uint8 *)pConveyor_s,sizeof(app_Conveyor_info_t)-sizeof(unsigned short),0xFFFF))
    {
        return;
    }
    ...
    if(!menu_Conveyor_Search_Inbox(pConveyor_s))
    {
        GetTimeChar(conveyor_s.time);
        GetDateChar(conveyor_s.date);
        conveyor_s.num=pConveyor_s->num;
        conveyor_s.status=pConveyor_s->status;
        conveyor_s.infotype=pConveyor_s->infotype;
        conveyor_s.malfunction_msg=pConveyor_s->malfunction_msg;
        conveyor_s.malfunction_type=pConveyor_s->malfunction_type;
        conveyor_s.speed=pConveyor_s->speed;
        conveyor_s.tensility=pConveyor_s->tensility;
        conveyor_s.head.seqnum=pConveyor_s->blk;

        flag=menu_Conveor_Add(&conveyor_s,CTYPE_INFO);
        if(flag==ZSUCCESS)
        {
            Menu_Set_SMS_Full_Ring_Flag(TRUE);
            Hal_RingStart(RING_ANY, OPENFLAG_ASSMS_POW);
            HalResetBackLightEvent();
            Menu_handle_msg(MSG_CONVEYOR_INFO_INCOMING, NULL, 0);
        }
    }
}
...
//#else
    else MP_DevInfo.DesireCoordPanID = 0xFFFF;
//#endif
