/**************************************************************************************************
  Filename:       ZGlobals.c
  Revised:        $Date: 2010/11/02 22:42:51 $
...
#if (defined MINE_PROJECTS) || (defined GASMONITOR_PROJECT)
#include "App_cfg.h"
#endif
...
#ifdef MENU_RF_DEBUG
{
  ...
  rtn = osal_nv_item_init( ZCD_NV_SET_CHANLIST, sizeof(uint32), NULL);
  
  if(rtn == NV_ITEM_UNINIT )
  {
        osal_nv_write(ZCD_NV_SET_CHANLIST, 0, sizeof(uint32), &zgDefaultChannelList);
  }
  else if(rtn == ZSUCCESS)
  {
       osal_nv_read( ZCD_NV_SET_CHANLIST, 0, sizeof(uint32), &zgDefaultChannelList );
  }
  else
  {
  ...
  rtn = osal_nv_item_init( ZCD_NV_SET_PANID, sizeof(uint16), NULL);
  
  if(rtn == NV_ITEM_UNINIT )
  {
        osal_nv_write(ZCD_NV_SET_PANID, 0, sizeof(uint16), &zgConfigPANID);
  }
  else if(rtn == ZSUCCESS)
  {
