...
#define CARDSEARCH_RESERVE_TIME     12   //second
#define  V2CARD_CHECK_CHANNEL   0x19
...
extern uint16 cardNum_u16;
extern pwrmgr_attribute_t pwrmgr_attribute;
static uint8 cardNumSearch_Siglevel;
static Time_t cardNumSearch_LastRcv_Time;

/* for card check */
#ifdef CFG_GAS_CARDCHECK
uint16       gas_cardcheck_num;
#endif
/*general functions */
...
#ifdef CFG_GAS_FIND_APPOINT_CARD
static void menu_find_appoint_card_display(void);
#endif
...
#endif
static void menu_CardNumSearch_result_display(void);
...
#endif
#ifdef  CFG_GAS_DETECTOR
ZEROADJUST_CHINA,CALIBRATION_CHINA,
#endif
    /*ACCIDENTALARM_CHINA,*/
#ifdef CFG_GAS_TEMPERATURE
    TEMPERATURE_CHINA,
...
static char*  const __code  ItemList_BacklightSettings[] = {CLOSE_CHINA,SECONDS_10_CHINA,SECONDS_20_CHINA,SECONDS_30_CHINA};//,NOLIMIT_CHINA};
    ...
    #else
    static char*  const __code  ItemList_SettingsList[] = {OVERDENSITY_CHINA, BACKLIGHTTIME_CHINA,TIMESETTING_CHINA};
    #endif
//#endif
//#ifndef CFG_GAS_DETECTOR
#else
    #ifdef CFG_GAS_TEMPERATURE
    static char*  const __code  ItemList_SettingsList[] = {BACKLIGHTTIME_CHINA,TEMPERATURE_CALIB_CHINA,TIMESETTING_CHINA};
    #else
    static char*  const __code  ItemList_SettingsList[] = {BACKLIGHTTIME_CHINA,TIMESETTING_CHINA};
    #endif
#endif
...
#endif
static char*  const __code  ItemList_SEARCHCARD[]= {V2CARDSEARCH_CHINA,V2CARDNUMSEARCH_CHINA};
...
static Tree_node_t  const __code Menu_Tree[] =
{
    ...
    #else
        .FirstChildID = MENU_ID_CARDSEARCH,
    #endif
#endif
    ...
    */
...
#endif
    {
        ...
        .ParentID = MENU_ID_FUNCTIONLIST,
        .FirstChildID =MENU_ID_V2CARDSEARCH,
        .ChildNum = sizeof(ItemList_SEARCHCARD)/sizeof(ItemList_SEARCHCARD[0]),
        .ItemName_CH = ItemList_SEARCHCARD,
        .oper.display = menu_list_display,
...

    {
        ...
        .name = SETTINGS_CHINA,
        .ParentID = MENU_ID_FUNCTIONLIST,
        .FirstChildID = MENU_ID_SETTINGS_BACKLIGHT,
...
#ifdef CFG_GAS_FIND_APPOINT_CARD
    {
        .ID = MENU_ID_FIND_APPOINT_CARD,
        .name = FIND_APPOINT_CARD_CHINA,
        .ParentID = MENU_ID_FUNCTIONLIST,
        .FirstChildID = NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
        .oper.display = menu_find_appoint_card_display,//menu_list_display
        .oper.on_key = menu_find_appoint_card_onkey,
...
#endif
{
    ...
    ,
    {
        .ID = MENU_ID_V2CARDSEARCH,
        .name = V2CARDSEARCH_CHINA,
    ...
    ,
    {
        .ID = MENU_ID_V2CARDNUMSEARCH,
        .name = V2CARDNUMSEARCH_CHINA,
        .ParentID = MENU_ID_CARDSEARCH,
        .FirstChildID =NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
        .oper.display = menu_CardNumAdjust_display,
        .oper.on_key = menu_CardNumAdjust_onkey
    }
    ,
    {
        .ID = MENU_ID_V2_CARDSEARCH_RESULT,
        .name =NULL,
        .ParentID = MENU_ID_V2CARDSEARCH,
        .FirstChildID = NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
    ...
    ,
    {
        .ID = MENU_ID_V2_CARDNUMSEARCH_RESULT,
        .name = NULL,
        .ParentID = MENU_ID_V2CARDNUMSEARCH,
        .FirstChildID = NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
        .oper.display = menu_CardNumSearch_result_display,//menu_list_display
        .oper.on_key = menu_CardSearch_result_onkey,
    }
};
...
static void    menu_selectlist_display(uint8* pVal)
{
    ...
    if(strlen((char*)pVal) <= LCD_LINE_WIDTH-2)
    {
        LCD_Str_Print(pVal, LCD_LINE_WIDTH - osal_strlen((char *)pVal)-2, 3, TRUE);
    }
    else
    {
        LCD_Memory_Print(pVal, LCD_LINE_WIDTH-2, 0, 2);
        LCD_Memory_Print(pVal+LCD_LINE_WIDTH-2, osal_strlen((char *)pVal)-(LCD_LINE_WIDTH-2), 0, 3);
    }
...
static void menu_shortmessage_read_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_UP:
        ...
        break;
    case HAL_KEY_DOWN:
        if(gassms_info.smsStartLine < lineCnt-SCREEN_LINES)
        {
            LCD_Clear(0, 1, LCD_LINE_WIDTH, 3);
            gassms_info.smsStartLine++;
            GasSms_LCDPrint(gassms_buf.p, gassms_buf.len, gassms_info.smsStartLine, 1, SCREEN_LINES+1, LCD_LINE_WIDTH);
...
static void menu_tools_display(void)
{
    uint8 p[LCD_LINE_WIDTH*2-3] = {'\0'};   //最多2行，每行14位
    ...
    {
        if(strlen(GASMONITOR_SW_VERSION) <= LCD_LINE_WIDTH*2-4)
        {
            strcpy((char *)p, GASMONITOR_SW_VERSION);
        }
        else
        {
            memcpy((char *)p, GASMONITOR_SW_VERSION, LCD_LINE_WIDTH*2-4);
        }
    }
...
static void menu_find_appoint_card_display(void)
{
    ...
    LCD_Str_Print((uint8 *)FIND_APPOINT_CARD_CHINA, (LCD_LINE_WIDTH-osal_strlen((char *)OVERDENSITYED_CHINA))/2, 0,TRUE);
    if(!is_achieve_cardnumber)
    {
        LCD_Str_Print((uint8 *)INPUT_CARD_NUMBER_CHINA, 0, 1,TRUE);
        LCD_Str_Print((uint8 *)":", osal_strlen((char *)INPUT_CARD_NUMBER_CHINA), 1,TRUE);
        GasMonitor_stationID_fromdensity(&n1,&n2,&n3,&n4, &n5, Get_appoint_card_number());
        a[0]=n1+'0';
        a[1]=n2+'0';
        a[2]=n3+'0';
        a[3]=n4+'0';
        a[4]=n5+'0';
        a[5]='\0';
        state=Get_adjustgas_invpos();
        LCD_Str_Print((uint8 *)a, 5, 2,TRUE);
    ...
    {
        LCD_Str_Print((uint8 *)URGENT_THE_CARD_CHINA, 0, 2,TRUE);
    }
}
#endif
...
static void menu_CardCheck_display(void)
{
   if( MENU_ID_CARDCHECK==CurrentNodeID)
    {
           uint8 tmp8 = V2CARD_CHECK_CHANNEL;//med_DevInfo.ExitAddr[EXT_MACADDR_CHANNEL];
...
static void menu_CardCheck_onkey(uint8 keys, uint8 status)
{
   if( MENU_ID_CARDCHECK==CurrentNodeID)
    {
           uint8 tmp8 =  aExtendedAddress[EXT_MACADDR_CHANNEL];
           MAC_MlmeSetReq(MAC_LOGICAL_CHANNEL, &tmp8);
    }
    ...
    switch(keys)
    {
        case HAL_KEY_MODE:
        {
            osal_stop_timerEx(Hal_TaskID, GASMONITOR_CHECKCARD_TIMER_EVENT);  // stop gas ADC
            osal_unset_event(Hal_TaskID, GASMONITOR_CHECKCARD_TIMER_EVENT);
...
static void menu_CardNumSearch_result_display(void)
{
    char* cardFinding = "正在搜索";
    char* cardNotFinded = "未搜到";

    uint8 Siglevelprintbuf[4];
    char cardNumInfo[12] = {'\0'};

    static uint8 searchCounter = 0;

    if( MENU_ID_V2_CARDNUMSEARCH_RESULT==CurrentNodeID)
    {
        uint8 tmp8 = V2CARD_CHECK_CHANNEL;
        MAC_MlmeSetReq(MAC_LOGICAL_CHANNEL, &tmp8);
    }
    else
        return;
    ...
    {
        searchCounter = 0;
        cardNumSearch_Siglevel = 0xff;
        pwrmgr_attribute.pwrmgr_device = PWRMGR_ALWAYS_ON;

        LCD_Str_Print((uint8 *)CARDSEARCH_CHINA, 3, 0, TRUE);
        sprintf(cardNumInfo, "定位卡  %05u", cardNum_u16);
        LCD_Str_Print((uint8 *)cardNumInfo, 0, 1, TRUE);
    }
    else
    {
        LCD_Line_Clear(2);
    ...
    if(cardNumSearch_Siglevel != 0xff)
    {
        searchCounter = 0;

        memset(Siglevelprintbuf, 0, sizeof(Siglevelprintbuf));
        sprintf((char*)Siglevelprintbuf, "%1u",cardNumSearch_Siglevel);
        LCD_Str_Print(SIGNAL_CHINA, 3, 2, TRUE);
        LCD_Str_Print((uint8 *)Siglevelprintbuf,10,2,TRUE);
    }
    else
    {
        LCD_Str_Print((uint8 *)cardFinding,0,2,TRUE);

        if(searchCounter%6 == 1)
        {
            LCD_Str_Print((uint8 *)".",strlen(cardFinding),2,TRUE);
        }
        else if(searchCounter%6 == 2)
        {
            LCD_Str_Print((uint8 *)"..",strlen(cardFinding),2,TRUE);
        }
        else if(searchCounter%6 == 3)
        {
            LCD_Str_Print((uint8 *)"...",strlen(cardFinding),2,TRUE);
        }
        else if(searchCounter%6 == 4)
        {
            LCD_Str_Print((uint8 *)"....",strlen(cardFinding),2,TRUE);
        }
        else if(searchCounter%6 == 5)
        {
            LCD_Str_Print((uint8 *)".....",strlen(cardFinding),2,TRUE);
        }

        if((searchCounter%20 < 5)&&(searchCounter > 5))
        {
            LCD_Str_Print((uint8 *)cardNotFinded, 0, 3, TRUE);//LCD_LINE_WIDTH-1-strlen(cardNotFinded)
        }

        if(++searchCounter >= 6*20)
            searchCounter = 0;
    }

    NearLastNodeID = CurrentNodeID;
    GasMonitor_StartItselfMenuLibEvt(1000); //每一秒刷新一次
}
static void menu_CardSearch_result_display(void)
{
    if( MENU_ID_V2_CARDSEARCH_RESULT==CurrentNodeID)
    {
        uint8 tmp8 = V2CARD_CHECK_CHANNEL;//med_DevInfo.ExitAddr[EXT_MACADDR_CHANNEL];
        MAC_MlmeSetReq(MAC_LOGICAL_CHANNEL, &tmp8);
    }
    ...
    {
        LcdClearDisplay();
        LCD_Str_Print((uint8 *)CARDSEARCHED_CHINA, 2, 0, TRUE); //第一行显示“共搜到 张”
    }
    else
    {
        for(uint8 i=1; i<SCREEN_LINES+1; i++)
    ...
    {
        sprintf((char*)IDprintbuf, "%02u",CardInfo.IDnum);  //卡总数，填入显示的第一行。
        LCD_Clear(8, 0, 9 , 0);
    ...
    if(node.ChildNum <= SCREEN_LINES)//卡数量少于3
    {
        for(uint8 i=0; i<node.ChildNum - node_info.show_item; i++)
        {
            if(CardInfo.CardSiglevel[node_info.show_item+i] == 0xff)
            {
                sprintf((char*)IDprintbuf, "%u",CardInfo.CardID[node_info.show_item+i]);

                if(IDprintbuf!=0)
                {
                    LCD_Clear(0, i+1, 15, i+1);
                    LCD_Str_Print(IDprintbuf, 0, i+1, TRUE);
                    LCD_Str_Print(LOSESIGNAL_CHINA,6,i+1, TRUE);
                }
            }
            else
            {
                ...
                {
                    LCD_Str_Print(IDprintbuf, 0, i+1, TRUE);
                    LCD_Str_Print(SIGNAL_CHINA,6,i+1, TRUE);
    ...
    {
        for(uint8 i=0; i<SCREEN_LINES; i++)
        {
            if(CardInfo.CardSiglevel[node_info.show_item+i] == 0xff)
            {
                sprintf((char*)IDprintbuf, "%u",CardInfo.CardID[node_info.show_item+i]);
...
{
    if( MENU_ID_V2_CARDSEARCH_RESULT == CurrentNodeID
        || MENU_ID_V2_CARDNUMSEARCH_RESULT == CurrentNodeID)
    {
        uint8 tmp8 = aExtendedAddress[EXT_MACADDR_CHANNEL];
        MAC_MlmeSetReq(MAC_LOGICAL_CHANNEL, &tmp8);
    }
...
{
    if(CardID == 0 || CardID >65000)
        return;

    if(CurrentNodeID==MENU_ID_V2_CARDSEARCH_RESULT)
    {
        uint8 CardSiglevel  = GasMonitor_RSSI2Level(rssi);
        menu_FillCardInfo(CardID,CardSiglevel);
    }
    if(CurrentNodeID==MENU_ID_V2_CARDNUMSEARCH_RESULT)
    {
        if(CardID == cardNum_u16)
        {
            cardNumSearch_Siglevel  = GasMonitor_RSSI2Level(rssi);
            cardNumSearch_LastRcv_Time = GetTime();
        }
    }

#ifdef CFG_GAS_CARDCHECK
    if((CurrentNodeID == MENU_ID_CARDCHECK)
        && rssi > -12
        )  // for card check
    {
        if(CardID != gas_cardcheck_num)  // a new card  comes
        {
            ...
            //启动事件，1200ms后把gas_cardcheck_num复位
            if(ZSuccess != osal_start_timerEx(GasMonitor_TaskID, GASMONITOR_CHECKCARD_TIMER_EVENT, 1200))
            {
                SystemReset();
            }
        }
...
bool menu_FillCardInfo(uint16 shortAddr, uint8 CardSiglevel)
{
    ...
    if(CardInfo.IDnum< MAXIDNUM)
    {
        ...
        CardInfo.RcvTime[CardInfo.IDnum]=GetTime();
        CardInfo.IDnum++;
...
void menu_CardSearch_start(void)
{
    ...
    CardInfo.RcvTime= (Time_t*)osal_mem_alloc(sizeof(Time_t)*MAXIDNUM);

    if(CardInfo.CardID == NULL
        ||CardInfo.CardSiglevel == NULL
        ||CardInfo.RcvTime == NULL)
    {
        SystemReset();
    }
    CardInfo.IDnum = 0;
...
void menu_CardSearch_end(void)
{
    ...
    osal_mem_free(CardInfo.RcvTime);
    CardInfo.IDnum=0;
}

void updataSearchCardOvertime(void)
{
    if(CurrentNodeID == MENU_ID_V2_CARDNUMSEARCH_RESULT)
    {
        isSearchCardOverTimed(cardNumSearch_LastRcv_Time,&cardNumSearch_Siglevel);
    }
    else if(CurrentNodeID == MENU_ID_V2_CARDSEARCH_RESULT)
    {
        Time_t timeNow = GetTime();
        for(uint8 i=0; i<CardInfo.IDnum; i++)
        {
            isSearchCardOverTimed(CardInfo.RcvTime[i],&CardInfo.CardSiglevel[i]);
        }
    }
}

bool isSearchCardOverTimed(Time_t lastRcvTime, uint8* pSiglevel)
{
    if(*pSiglevel == 0xff)
        return false;
    ...
    if(nowTime.min == lastRcvTime.min)
    {
        if(nowTime.sec - lastRcvTime.sec >= CARDSEARCH_RESERVE_TIME)
        {
            *pSiglevel = 0xff;
        }
    }
    else if(nowTime.min - lastRcvTime.min == 1
        || (int16)nowTime.min - (int16)lastRcvTime.min == -59
        )
    {
        if(nowTime.sec + 60 - lastRcvTime.sec >= CARDSEARCH_RESERVE_TIME)
        {
            *pSiglevel = 0xff;
        }
    }
