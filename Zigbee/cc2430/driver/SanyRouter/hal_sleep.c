/**************************************************************************************************
  ...
  Revised:        $Date: 2009-05-19 11:22:32 -0700 (Tue, 19 May 2009) $
  Revision:       $Revision: 19985 $
...
/*
 * When sleeping in PM1, the sleep timer needs to be used
 * to periodically wake up and clear the watchdog if enabled.
 * This constant sets number of 32kHz cycles before the WDT
 * times out to set the sleep timer interrupt to.
 */
#define HAL_SLEEP_WDT_OVERLAP            4
 ...
 */
#define EXTERNAL_INTERRUPT_DELAY()          st( if( CHVER == REV_D )                                  \
                                                {                                                     \
                                                  P0DIR |= BV(1);    /* set P0.1 output */            \
...
#ifdef WDT_IN_PM1
uint16 halSleepGetWdtPeriod( void );
#endif
...
void halSleep( uint16 osal_timeout )
{
...
#endif
  {
    ...
    if (MAC_PwrOffReq(MAC_PWR_SLEEP_DEEP) == MAC_SUCCESS)
    {
...
#ifdef WDT_IN_PM1
      wdtPeriod = halSleepGetWdtPeriod();
				
      if ((timeout == 0) && (halPwrMgtMode == CC2430_PM1) && (WDCTL & WD_EN) && (WDCTL & WD_MODE) == 0)
      {
        /* Enable sleep timer to wake up before WDT times out to ensure that a WDT reset is not
         * generated immediately after waking up from an external interrupt.
         */
        timeout = wdtPeriod - HAL_SLEEP_WDT_OVERLAP;
      }
#endif
...
#ifdef WDT_IN_PM1
        /* WDT_IN_PM1 is a workaround for watchdog timer. The watchdog timer can be used in
         * PM2 and PM3 without a problem. But when it is used with PM1, the device must wake
         * up frequently to kick the watchdog.
         */
        else if ((halPwrMgtMode == CC2430_PM1) && (WDCTL & WD_EN) && ((WDCTL & WD_MODE) == 0) &&
                 (timeout > (wdtPeriod - HAL_SLEEP_WDT_OVERLAP)))
        {
          /* If watchdog is enabled, one has to wake from PM1 to clear the timer before it times out. */
          timeout -= (wdtPeriod - HAL_SLEEP_WDT_OVERLAP);
          halSleepSetTimer(wdtPeriod - HAL_SLEEP_WDT_OVERLAP);
       ...
       * Osal clock adjustment has been moved to macSleepWakeUp() function
       * to reduce the wake up time from sleep.
       */
      MAC_PwrOnReq();
    }
    HAL_EXIT_CRITICAL_SECTION(intState);
...
extern uint16 sleeps;
extern uint32 sleep32kticks;
#endif
uint32 HalTimerElapsed( void )
{
  uint32 ticks;
...
#ifdef SLEEPTIME_ACCOUNTING
  sleeps++;
  sleep32kticks += ticks - HAL_SLEEP_ADJ_TICKS;
#endif
...
#ifdef WDT_IN_PM1
/**************************************************************************************************
 * @fn          halSleepGetWdtPeriod
 *
 * @brief       Get the watchdog timer period.
 *
 * input parameters
 *
 * @param       None
