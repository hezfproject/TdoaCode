...
#define UART_STATE_PAYLOAD  3

static uint8  state = UART_STATE_IDLE;
static uint8  data_len=0;
static uint8  header_count=0;
static uint8  save_cmdtype=0;
static uint8  start_byte =0; 
static void Reset_state_machine(void);
static void Get_data_form_UART_buff(uint8  startbyte, uint8 len );
/*********************************************************************
 * GLOBAL FUNCTIONS
 */
...
HAL_ISR_FUNCTION( halUart0RxIsr, URX0_VECTOR )
{
        ...
        data = cfg0->rxBuf[cfg0->rxHead];

        // sizeof(Uart_Header_t) ==8  
        //     1             1                    2             1                   1             2
        //header_h  header_l         crc         cmdtype          len        padding        
        ...
        switch(state)
        {
            ...
            {        
                 ...
                 else if(header_count == 6)
                 {
                     ...
                     if(!uartWantReadLen||(uartWantReadLen >= (GASLOCATE_UART_MAX_STRUCT_LEN+1)))
                     {
                           //if some msgtype  no payload
                            if( !uartWantReadLen)
                            {
                                   Get_data_form_UART_buff(start_byte,0);  
                                   GasMonitor_start_UART0_event();
                            }
                            state =  UART_STATE_IDLE;
                     }
                     else
         ...
         case UART_STATE_PAYLOAD:
         {
                    ...
                    {    
                        ...
                        if(!uartWantReadLen)
                        {
                              Get_data_form_UART_buff(start_byte,data_len);  
                              GasMonitor_start_UART0_event();
                              state =  UART_STATE_IDLE;
                        }
                    }
...
static void Get_data_form_UART_buff(uint8  startbyte, uint8 len )
{
      if(startbyte >= (cfg0->rxMax-sizeof(Uart_Header_t)-len+1))
      {
                osal_memcpy(receive_uart_buff,&cfg0->rxBuf[startbyte],cfg0->rxMax-startbyte);                           
                osal_memcpy((uint8*)receive_uart_buff+cfg0->rxMax-startbyte,&cfg0->rxBuf[0],len+sizeof(Uart_Header_t)-(cfg0->rxMax-startbyte));
       }
       else
       {
                osal_memcpy(receive_uart_buff,&cfg0->rxBuf[startbyte],len+sizeof(Uart_Header_t));
       }
}
