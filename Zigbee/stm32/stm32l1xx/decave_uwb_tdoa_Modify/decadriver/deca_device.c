...
void dwt_setdblrxbuffmode(int enable)
{
    if(enable)
    {
        ...
        dw1000local.sysCFGreg &= ~SYS_CFG_DIS_DRXB; //取反后为0进行开启双缓冲区接收器
...
extern uint8 gu8NblinkPack;
void dwt_sethsrb(void)
{
	//切换接收缓冲区指针，使用另外一个缓冲区进行接收
	uint8 hsrb = 0x1 ;
	//toggle the host side Receive Buffer Pointer by writing one to the register 向第三字节写入一个字节长度的数据hsrb，设置为1说明开启切换命令
...
void dwt_isr(void) // assume interrupt can supply context
{
	...
	//PrintfUtil_vPrintf("status = %d   dwt_isr\n", status);
    ...
    if (status & SY_STAT_RFCG)  // Receiver FCS Good
    {
        ...
        if(dw1000local.dwt_rxcallback != NULL)
         ...
         if((dw1000local.dblbuffon) && (gu8NblinkPack == 0))  //double RX buffer is enabled - toggle the host side pointer
        {
       		 //PrintfUtil_vPrintf("0\n");
        	//切换接收缓冲区指针，使用另外一个缓冲区进行接收
            uint8 hsrb = 0x1 ;
            //toggle the host side Receive Buffer Pointer by writing one to the register 向第三字节写入一个字节长度的数据hsrb，设置为1说明开启切换命令
            dwt_writetodevice(SYS_CTRL_ID, 3, 1, &hsrb) ;       // we need to swap rx buffer status reg (write one to toggle internally)
			//设置缓冲区指针后，开启缓冲区自动接收
//new123			dwt_setautorxreenable(1);
		}
