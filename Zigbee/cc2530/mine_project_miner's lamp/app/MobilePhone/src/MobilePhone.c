...
uint16 MP_ProcessEvent(uint8 taskId, uint16 events)
{
    ...
    if(events & MP_DIALUP_RETRY_EVENT)
    {
                ...
                && (ON_CALLING() || ON_CALLINGWAIT()))
        {
            ...
            //MP_SendCmd(MP_UP_DIALUP, &MP_AudioInfo.peer_termnbr, 20);
    ...
    if(events & MP_CLOSE_RETRY_EVENT)
    {
        if(++MP_AudioInfo.close_cnt < 3 && MP_AudioInfo.peernmbr != MP_SHORT_INVALIDNMMBR)
        {
            ...
            MP_start_timerEx(MP_TaskId, MP_CLOSE_RETRY_EVENT, 200);
        ...
        else
        {
            ...
            MP_AudioInfo.retrying_bitmap &= ~MP_CLOSE_RETRY_BIT;
            MP_ResetAudio();//resetaudio must after  sendcmd   MP_UP_CLOSE
...
//#endif
	if(events & MP_HANG_UP_TIMEOUT_EVENT)
	{
    	Phone->huangupstatus = WAITOFF;
    	return events ^ MP_HANG_UP_TIMEOUT_EVENT;
	}
    ...
    if(events & MP_UART_READ_EVENT)
    {
        ...
        {
            uartlen = HalUARTRead(HAL_UART_PORT_0,uartReadBuf,63);
            if(uartlen > 0)
            {
                if(uartReadBuf[0] == 1)
                {
                ...
                MP_start_timerEx(MP_TaskId, MP_UART_READ_EVENT, 4000);
                return events ^ MP_UART_READ_EVENT;
            }
        }

        MP_start_timerEx(MP_TaskId, MP_UART_READ_EVENT, 200);
        return events ^ MP_UART_READ_EVENT;
    }
    return 0;
}
    ...
    if(pHeader->protocoltype != APP_PROTOCOL_TYPE_MOBILE  || data.len < sizeof(app_header_t) + pHeader->len)
    {
        if(pHeader->protocoltype == APP_PROTOCOL_TYPE_CARD)
        {
            if(pHeader->msgtype == 0x05) // rssi
            {
                ...
                app_rssi_data.tof_rssi[u16RssiNum].u16ShortAddr = shortAddr;
                app_rssi_data.tof_rssi[u16RssiNum].u16SeqNum = psAppPkt->rf_tof_card_data.u16SeqNum;
                app_rssi_data.tof_rssi[u16RssiNum].i8Rssi = rssi;
                ...
                if(APP_MAX_CARD_NUM == u16RssiNum)
                {
                    ...
                    MP_SendTunnelToAllCoord((uint8 *)(&app_rssi_data),u16RssiNum*sizeof(tof_rssi_ts),TUNNEL_PAYLOAD,true);
                }
                return ZSUCCESS;
            }
        }
...
void MP_ParseVoice(const app_mpVoice_t *pVoice, int8 rssi)
{
  ...
  if((MP_AudioInfo.peernmbr==0)||(MP_AudioInfo.peernmbr==0xffff))
    MP_AudioInfo.peernmbr=pVoice->hdr.srcaddr;
  switch(Phone->whichphone)
  {
    case CARDphone:
      if((Phone->huangupstatus==WAITON) || MP_DevInfo.nmbr != pVoice->hdr.dstaddr
          || (pVoice->hdr.srcaddr != MP_AudioInfo.peernmbr &&(MP_AudioInfo.peernmbr != MP_SHORT_GATEWAYNMBR)))
      {
        return;
      }
      if(IS_IDLE() || ON_WAKE())
      {
        ...
        shamcmd=osal_mem_alloc(sizeof(app_mpCmd_t));
        shamcmd->cmdtype=MP_DOWN_CALL;
        shamcmd->dstnbr=MP_DevInfo.termNbr;
		Phone->callstatus=CALLED;
        MP_ParseCmd(shamcmd);
        osal_mem_free(shamcmd);
      }
  ...
  //if(pVoice->seqnum > MP_AudioInfo.seqnum|| (pVoice->seqnum != MP_AudioInfo.seqnum
      //&&  MP_AudioInfo.seqnum > 245 && pVoice->seqnum < 10))
  ...
  if(pVoice->seqnum != MP_AudioInfo.seqnum)
  {
	  ...
	  debugnum3[testi3++] =pVoice->seqnum;
      ...
      //if((MP_AudioInfo.seqnum+1)!=pVoice->seqnum)
      //{
        //seqnumerrcnt++;
      //}
...
//#endif
	...
	debugnum1[testi1++]=*(uint16 *)MP_AudioInfo.peer_termnbr.nbr;
    ...
    case MP_DOWN_ACCEPT:
    {
        if(num_isequal(&MP_AudioInfo.peer_termnbr, &pCmd->srcnbr))  /* command from peer */
        {
            ...
            {
                MP_SendCmd(MP_UP_CLOSE, &pCmd->srcnbr, MP_AudioInfo.cmdseqnum++);
            }
        }
        else
        {
    ...
    case MP_DOWN_CALL:
    {
        if(ON_CALLING() || ON_CALLINGWAIT() || ON_FOUND() || ON_CALLED() || ON_AUDIO())
        {
            ...
            {
                MP_SendCmd(MP_UP_FOUND, &pCmd->srcnbr, MP_AudioInfo.cmdseqnum++);
                HalResetBackLightEvent();
            }
            else     //a new comer and refuse its calling.
            {
                MP_SendCmd(MP_UP_BUSY, &pCmd->srcnbr, MP_AudioInfo.cmdseqnum++);
            }
        }
...
static  uint8  MP_ReadExtAddr(void)
{
    ...
    {
        channel = 25;   //use default channel 25;
    }
