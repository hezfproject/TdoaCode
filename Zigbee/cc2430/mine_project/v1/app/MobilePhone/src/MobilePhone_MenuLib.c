...
uint8 Menu_Init(void)
{
    ...
    Buffer_Init(&dialnum_buf, NMBRDIGIT);
    Menu_Stack_Init();
    menu_nv_init();
...
static uint8     menu_nv_init()
{
    ...
    if(temp == NV_ITEM_UNINIT)
    {
        ...
        displayparam.paramTen =0;
        displayparam.paramTwelve =0;
    ...
    {
        len = 0;
        osal_nv_write(MP_STOREPARAM_ITEM,0, sizeof(uint8), &len);
    }
    else if(temp == NV_OPER_FAILED)
    {
        return NV_OPER_FAILED;
...
uint8 Save_New_SMS(app_SMS_t *p)
{
    uint16 len;
    ...
    len = NMBRDIGIT + SMS_LEN_TYPE_SIZE + p->len+sizeof(uint16);

#ifdef NEW_DOUBLE_NVID_OP
    //rtn = menu_doubleNVID_write(SMS_NVID, offset, len, &p->len);
    rtn = menu_doubleNVID_write(SMS_NVID, offset, len, &p->blk);
#endif
