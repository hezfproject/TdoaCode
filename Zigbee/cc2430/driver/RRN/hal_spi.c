...
void HalSpiSendMsg(void)
{
    ...
    {
       uint8* pMsg = NULL;
    	if((pMsg = (uint8 *)osal_msg_allocate(spicfg.bufSize+10)) != NULL)
    		{
        		uint16 len;
        		if(HalSPIRead((uint8*)pMsg, &len) == ZSUCCESS)    
        			{
					//Spi_SendEvt(pMsg);
				if(*pMsg!=SPI_RX)*pMsg=SPI_RX;	
				osal_msg_send(spicfg.taskID, (byte *)pMsg);	
				}
    		}
       // if(ZSUCCESS!= osal_set_event(spicfg.taskID, spicfg.eventID))
        //{
        //    SystemReset();
       // }
    }
