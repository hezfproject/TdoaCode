...
PUBLIC void AppColdStart(void)
{
	...
	TimerUtil_eSetCircleTimer(STATION_FEED_WATCHDOG_EVENT, 1000);
...
PRIVATE void vGet_MACAddress(void)
{
	...
	if(u8Channel < 11 || u8Channel > 26)
	{
		...
		{
		u8Channel = DEFAULT_CHANNEL_RSSI;
		}
	}
}
	...
	while(event && count++<32)
	{
		...
		case STATION_FEED_WATCHDOG_EVENT:
		{
			...
			if(i++%2)
			{
				...
				if(current_vol >1485)
					Irms = current_vol*5 - 6930;
				else if(current_vol >1453)
					Irms = current_vol*5 - 6855;
				else if(current_vol >1426)
					Irms = current_vol*5 - 6850;
				else if(current_vol >1399)
					Irms = (current_vol*10)/3 - 4358;
				else if(current_vol >1365)
					Irms = (current_vol*2) - 2530;
				else if(current_vol >1305)
					Irms = (current_vol/2) - 409;
				
				
				else if(current_vol >1194)
					Irms = current_vol/2 - 329;
				else if(current_vol > 951)
					Irms = (current_vol)/6 -9;
				
				else
					Irms = (current_vol/6) -39;
				//Irms = (current_vol*10 /17)*10 - 8670;
				//vAHI_DioSetOutput(LIGHT_KEY_PIN,0);
			}
		...
		case STATION_REPORT_STATUS_EVENT:
		{
			...
			pStationReport->u8Irms = Irms;
			pStationReport->u8Vrms = Vrms;
		...
		case STATION_REPORT_DATA_EVENT:
		{
			...
			vReportCardRssi();
			TimerUtil_eSetTimer(STATION_REPORT_DATA_EVENT, 5000);
