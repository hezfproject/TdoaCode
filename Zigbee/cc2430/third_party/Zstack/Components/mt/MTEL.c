/**************************************************************************************************
  ...
  Description:    MonitorTest Event Loop functions. Everything in the MonitorTest Task.
...
#ifdef ZPORT
const char *MTVersionString[] = {"1.00 (F8W1.4.3-ZP)", "1.10 (F8W1.4.3-ZP)"};
#else
const char *MTVersionString[] = {"1.00 (F8W1.4.3)", "1.10 (F8W1.4.3)"};
#endif
...
byte MT_ProcessSetNV( byte *pData )
{
  ...
  attrib = (uint16) *pData;
  pData += 2;   // skip additional byte containing len field
...
void MT_ProcessGetNV( byte *pData )
{
  ...
  if ( buf != NULL )
  {
    ...
    buf[0] = osal_nv_read( attrib, 0, attlen, &buf[3] );
    buf[1] = (uint8)attrib;
    buf[2] = attlen;
...
void MT_ProcessAppUserCmd( byte *pData) 						
{
  ...
  switch ( app_cmd )
  {
    ...
    case TP_SEND_UPDATE_KEY:
      ret = TestProfileApp_UpdateKey( srcEp, (uint8)param1, param2 );
      break;

    case TP_SEND_SWITCH_KEY:
      ret = TestProfileApp_SwitchKey(  srcEp, (uint8)param1, param2 );
      break;
