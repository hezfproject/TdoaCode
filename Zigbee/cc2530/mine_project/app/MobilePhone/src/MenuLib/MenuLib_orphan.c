...
static void    menu_dialing_display(void)
{
    ...
    Contact_Node c_node;
    if(ZSuccess == menu_Contact_SearchContactByNum(&c_node, NULL, dialnum_buf.p))
    {
...
static void    menu_talking_display(void)
{
    ...
    if(MP_AudioInfo.peer_termnbr.Nmbr!=NULL && (num_len = strlen(MP_AudioInfo.peer_termnbr.Nmbr))!=0)
    {
        if(ZSuccess == menu_Contact_SearchContactByNum(&c_node, NULL, MP_AudioInfo.peer_termnbr.Nmbr))
        {
            LCD_Str_Print(c_node.name, (16 - strlen((char*)c_node.name)) / 2, 2, TRUE);
        }
        else
        {
            LCD_Str_Print(MP_AudioInfo.peer_termnbr.Nmbr, (16 - num_len) / 2, 2, TRUE);
        }
    }
#ifdef CELLSWITCH_DEBUG
...
static void    menu_callrecord_detail_display(void)
{
        ...
        LCD_Str_Print((char *)record.num.Nmbr, 0, 1, TRUE);
    ...
    if(input_status == OUTPUT_STATUS)
    {
        ...
        else if(keys == HAL_KEY_SELECT)
        {
            if(data_buf.len > 0)
            {
                ...
                {
#ifdef NEW_DOUBLE_NVID_OP
                    if(strlen((char*)num_buf.p) < APP_NMBRDIGIT && strlen((char*)data_buf.p) < MAX_NAME_LEN)
                    {
                        Contact_Node Node;
                        strcpy((char*)Node.num.Nmbr, (char*)num_buf.p);
...
static void    menu_main_display(void)
{
    ...
    if(isneed_judge_sms==true)
    	{
    	menu_SMS_Read_unread(&unread_sms);
    	}
#endif
    if(unread_sms> 0)
    {
