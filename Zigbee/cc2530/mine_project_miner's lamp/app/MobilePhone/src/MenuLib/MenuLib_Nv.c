...
//static uint8 Contact_List_Make_nouse_False(uint16 head_item);
...
uint8 menu_Contact_SearchContactByNum(Contact_Node *pContactNode, uint8* pidx, const void* pNum);
bool Is_inContact_SearchContactByNum_and_item(Contact_Node *pContactNode,uint16 contact_item, const void* pNum);
...
//static uint8 Record_List_Make_nouse_False(uint16 head_item,uint16 item_base,uint16 item_end);
...
//static uint8 SMS_List_Make_nouse_False(uint16 head_item,uint16 item_base,uint16 item_end);
...
uint8 menu_SMS_Search_Inbox(app_mpSMS_t *pSMS);
...
uint8 Record_SMS_When_AddContact(Contact_Node *pContactNode,uint16 contact_item)
{
    ...
    for(uint16 i=MP_NV_DIALED_BASE; i<=MP_NV_ANSWERED_END; i++)
    {
        ...
        if((record.Contect_item_L==LIST_ITEM_NULL)
                &&(num_isequal((app_termNbr_t*)record.num.nbr, (app_termNbr_t*)pContactNode->num.nbr) == 1)
                //(strcmp((char *)record.num.nbr, (char *)pContactNode->num.nbr) == 0)
          )
        {
    ...
    for(uint16 i=start_item; i<=MP_NV_SMS_INBOX_END; i++)
    {
        ...
        if((sms.head.Contect_item_L==LIST_ITEM_NULL)
                &&(num_isequal((app_termNbr_t*)sms.head.nmbr.nbr, (app_termNbr_t*)pContactNode->num.nbr) == 1)
                //(strcmp((char *)sms.head.nmbr.nbr, (char *)pContactNode->num.nbr) == 0)
          )
        {
...
bool Is_inContact_SearchContactByNum_and_item(Contact_Node *pContactNode,uint16 contact_item, const void* pNum)
{
    ...
    //if(strcmp((char *)c_node.num.nbr, (char *)pNum) == 0)
    if(num_isequal((app_termNbr_t*)c_node.num.nbr, (app_termNbr_t*)pNum) == 1)
    {
        *pContactNode=c_node;
        return TRUE;
    }
...
uint8 menu_Contact_SearchContactByNum(Contact_Node *pContactNode, uint8* pidx, const void* pNum)
{
    ...
    for(i=0; i<=(MAX_CONTACT_NUM-1); i++)
    {
        ...
        //if(strcmp((char *)c_node.num.nbr, (char *)pNum) == 0)
        if(num_isequal(&c_node.num, (app_termNbr_t*)pNum) == 1)
        {
            if(pContactNode!=NULL)
            {
                *pContactNode = c_node;
...
uint8 Contact_List_Search_End_Valid(Contact_Node *pContactNode,uint16 *head_item,uint8 *pidx)
{
    ...
    *pContactNode=c_node;
    save_last_ture_item=*head_item;
    if(c_node.item_head.item_next==LIST_ITEM_END)return  ZSuccess;//Contact_List_Make_nouse_False(save_last_ture_item);
...
uint8 menu_SMS_Search_Inbox(app_mpSMS_t *pSMS)
{
    ...
    for(uint16 i=MP_NV_SMS_INBOX_BASE; i<=MP_NV_SMS_INBOX_END; i++)
    {
        ...
        if(flag==ZSuccess)
        {
            ...
            {
                if((num_isequal((app_termNbr_t*)sms.head.nmbr.nbr, (app_termNbr_t*)pSMS->srcnbr.nbr) == 1)
                        //(!strcmp((char *)sms.head.nmbr.nbr, (char *)pSMS->srcnbr.nbr))
...
uint8 SMS_List_Search_End_Valid(sms_saved_t *pSMS,uint16 *head_item,uint8 *pidx,sms_type smstype)
{
    ...
    *pSMS=sms;
    ...
    save_last_ture_item=*head_item;
    if(sms.head.item_head.item_next==LIST_ITEM_END)return  ZSuccess;//SMS_List_Make_nouse_False(save_last_ture_item,item_base,item_end);
