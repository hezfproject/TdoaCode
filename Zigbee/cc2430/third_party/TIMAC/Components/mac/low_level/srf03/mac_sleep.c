...
/* The OSAL timer adjustment after sleep should only be done with legacy OSAL timer code.
 * i.e. when separate OSAL hardware timer is used.
 */
#if (defined POWER_SAVING) && (defined HAL_TIMER) && (HAL_TIMER == TRUE)
  #define MAC_SLEEP_UPDATE_SYSTEM_TIME()  osal_adjust_timers()
#else
  #define MAC_SLEEP_UPDATE_SYSTEM_TIME()
#endif
 ...
 * @fn          macSleepWakeUp
 *
 * @brief       Wake up the radio from sleep mode. Also, updates the system
 *              time.
 *
 * @param       none
 *
...
void macSleepWakeUp(void)
{
  ...
  {
    /* turn on radio power */
    MAC_RADIO_TURN_ON_POWER_START();

    /* System time is updated while waiting for the radio to power up.
     * This reduces the overall wake up time from sleep.
     */
    MAC_SLEEP_UPDATE_SYSTEM_TIME();

    /* Blocks till the radio is turned ON. */
    MAC_RADIO_TURN_ON_POWER_DONE();
...
#if ( (MAC_SLEEP_STATE_AWAKE    != MAC_PWR_ON) ||  \
      (MAC_SLEEP_STATE_OSC_OFF  != MAC_PWR_SLEEP_LITE) ||  \
      (MAC_PWR_SLEEP_DEEP       != MAC_SLEEP_STATE_RADIO_OFF))
#error "ERROR! Defines must match."
#endif
