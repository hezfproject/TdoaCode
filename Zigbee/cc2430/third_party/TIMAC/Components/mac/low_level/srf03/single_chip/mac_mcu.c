...
/*
 * This variable is used for a chip bug workaround. The radio may lockup when 320us MAC timer
 * overflow and RX FIFO overflow occur at the same time while CSP is running. Note that this
 * rare lockup condition only happens during super heavy broadcast traffic. Once the CSP and
 * FSMSTATE locked up, there is no way out except PM2 or chip reset. When the radio is locked
 * up, the FSMSTATE is always in RX FIFO Overflow (0x11) and FIFOP is held high.
 */
static uint8 radio_stuck = 0;

/*
 * This function pointer is called when the radio is locked up. The chip will reset next.
 * The application can populate this function to save all the necessary info.
 */
void (* macRadioStuckCallback)(void) = NULL;
...
void macMcuInit(void)
{
  ...
  */
  ...
  /* Turn on RX FIFO overflow interrupt */
  IEN0 |= BV(0);
}
...
void macMcuOverflowSetCount(uint32 count)
{
  ...
  if (accumulatedOverflowCount >= count)
  {
  ...
  {
    accumulatedOverflowCount += (0xFFFFFFFF - count);
  }
...
uint16 macMcuPrecisionCount(void)
{
  ...
  /* must read T2OF0 first, this latches T2OF1. T2OF2 is discarded. */
  ((uint8 *)&overflowCount)[UINT32_NDX0] = T2OF0;
  ((uint8 *)&overflowCount)[UINT32_NDX1] = T2OF1;
...
HAL_ISR_FUNCTION( macMcuRfIsr, RF_VECTOR )
{
  ...
  else if ((RFIF & IRQ_FIFOP) & rfim)
  {
    ...
    do
    {
      ...
      /* radio stuck detection code */
      if (FSMSTATE == 0x11)
      {
        radio_stuck++;
        if (radio_stuck > 100)
        {
          if (macRadioStuckCallback != NULL)
          {
            (*macRadioStuckCallback)();
          }
          HAL_SYSTEM_RESET();
        }
      }
  ...
  /*
   *  CC2430 chipbug 297 workaround - write 0xFF to the flag register. This normally
   *  has no effect, but in this case it will make the interrupt controller
   *  perform a new check whether to set the RFMISC interrupt or not.
   */
  S1CON = 0x00;
  RFIF = 0xFF;
}
