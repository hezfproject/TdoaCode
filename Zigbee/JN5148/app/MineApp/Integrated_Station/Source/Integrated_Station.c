...
PRIVATE void vReportThroughData(uint16 u16ShortAddr,MAC_McpsIndData_s *psMcpsInd);
PRIVATE void vReportVehicleData(MAC_McpsIndData_s *psMcpsInd);
...
PRIVATE void vInitSystem(void)
{
    ...
    */
	...
	SaveParam_t save_channel;

    if(bAHI_FullFlashRead(PARAM_STORE_ADDR, sizeof(SaveParam_t),(uint8*)&save_channel)
            && bChannelValid(save_channel.u8Channel))
    {
        u8Channel = save_channel.u8Channel;
    }
...
PRIVATE void vCheckSetting(void)
{
    ...
    if(u8Channel < 11 || u8Channel > 25
            || u16StationPanId < 20000 || u16StationPanId > 59999)
    {
...
PRIVATE void vProcessAppEventQueues(void)
{
    ...
    while(event && count++<32)
    {
        ...
        case STATION_REPORT_STATUS_EVENT:
        {
            ...
            u8StationPort = Jbsmac_u8GetRssiPort();
    ...
    case APP_PROTOCOL_TYPE_CARD:
    {
        ...
        if((psFrame->sDstAddr.u16PanId == 0xFFFF) || (psFrame->sDstAddr.u16PanId == u16StationPanId))
        {
            ...
            case TOF_STATION_RSSI_CHECK:
            {
                ...
                {
                    //i2c_vPrintf("PortB:%d\n",u8StationPort);
                    i8MpRcvRssi = SysUtil_vConvertLQI2Dbm(psFrame->u8LinkQuality);
                    vAHI_StartRandomNumberGenerator(E_AHI_RND_SINGLE_SHOT, E_AHI_INTS_DISABLED);
                    TimerUtil_vDelay((u16AHI_ReadRandomNumber() & 100), E_TIMER_UNIT_MILLISECOND);
                    vStationCast(TOF_STATION_RSSI_CHECK_ACK, 0xFFFF, 0xFFFF, 0, 0);
                    //Hal_SendDataToAir((uint8 *)u32CommonBufUp, 2, MOBILEPHONE_NWK_ADDR, u16_DstAddr, TRUE);
                }
                break;
            }
            case TOF_STATION_THROUGH:
            {

                vReportThroughData(psFrame->sSrcAddr.uAddr.u16Short,&psMcpsInd->uParam.sIndData);
                break;
            }
            case VEHICLE_CARD_RSSI:
            {
                //i2c_vPrintf("V rssi\n");
                vReportVehicleData(&psMcpsInd->uParam.sIndData);
                break;
            }
            default:
            {
...
PRIVATE void vBsMac_rx_callback(unsigned char *pbuf, unsigned char len)
{
    ...
    if(pheader->msgtype > MP_MP2ARM_CMDSTART && pheader->msgtype < MP_MP2MP_CMDEND)
    {
        ...
        //memcpy((uint8 *)u32CommonBufUp,(uint8 *)pheader,datalen);
        //u8LengthUp = datalen;

        //u16_DstAddr = u16DstAddr;

        //uint8 random_time;

        //vAHI_StartRandomNumberGenerator(E_AHI_RND_SINGLE_SHOT, E_AHI_INTS_DISABLED);
        //random_time = (u16AHI_ReadRandomNumber() & 0xF);
        //EDBG(i2c_vPrintf(" NUM %d\n",random_time););


        //TimerUtil_eSetTimer(STATION_UP_RADIO_DATA_EVENT, random_time);
        //i2c_vPrintf("D\n");

        Hal_SendDataToAir((uint8 *)pheader, datalen, u16DstAddr, 0x0000, TRUE);
	...
	/*else if(pheader->msgtype == MP_TRANSFER_THROUGH)
	{
    ...
    {
        app_transfer_through_t * p_through_header = (app_transfer_through_t *)psAppPkt;
        MacUtil_SendParams_s sParams;

        app_header_t * p_aap_header = (app_header_t *)u8ThroughBuf;

        p_aap_header->protocoltype = APP_PROTOCOL_TYPE_CARD;
        p_aap_header->msgtype = TOF_STATION_THROUGH;
        p_aap_header->len = p_through_header->device_through_data.len;
        CONVERT_ENDIAN(p_aap_header->len);

        i2c_vPrintf("CCCCC%d\n",p_aap_header->len);
        memcpy((uint8 *)u8ThroughBuf + sizeof(app_header_t),(uint8*)(p_through_header+1),p_aap_header->len);

        uint16 u16DstAddr = p_through_header->device_through_data.device_addr;
        CONVERT_ENDIAN(u16DstAddr);
        ...
        MacUtil_vSendData(&sParams, u8ThroughBuf, p_aap_header->len + sizeof(app_header_t),0);
    }
    break;
	}
    #if 0
...
PRIVATE void vWriteData2Stm(uint8* pbuf)
{
    ...
    {
        app_transfer_through_t* pDataHead = (app_transfer_through_t*) pbuf;
        vConvertEndian16(&(pDataHead->device_through_data.device_addr));
        vConvertEndian16(&(pDataHead->device_through_data.len));
	    break;
	}
    case APP_VEHICLE_CARD_RSSI:
    {
        //todo:card is 2530,so is little endian
        uint16 nwk_len;
        vConvertEndian16(&(u16App_len));
        nwk_len = u16App_len + sizeof(app_header_t) + sizeof(struct nwkhdr);
        pnwk_data->len = u16App_len + sizeof(app_header_t);
...
PRIVATE void vReportThroughData(uint16 u16ShortAddr,MAC_McpsIndData_s *psMcpsInd)
{
    app_transfer_through_t *pThHdr = (app_transfer_through_t *)u8ThroughUpBuf;
    MAC_RxFrameData_s *psFrame;
    psFrame = &psMcpsInd->sFrame;

    app_header_t *pheader = (app_header_t *)(psFrame->au8Sdu);
    uint8 * p = (uint8*)(pheader+1);

    pThHdr->hdr.protocoltype = APP_PROTOCOL_TYPE_CARD;
    pThHdr->hdr.msgtype = APP_TRANSFER_THROUGH;

    pThHdr->device_through_data.device_addr = u16ShortAddr;
    pThHdr->device_through_data.len = pheader->len;

    pThHdr->hdr.len = sizeof(app_device_through_ts)+pThHdr->device_through_data.len;

    memcpy(((uint8 *)u8ThroughUpBuf+sizeof(app_transfer_through_t)) , p, pheader->len);

    //EDBG(i2c_vPrintMem((uint8*)u8ThroughUpBuf, pThHdr->device_through_data.len+sizeof(app_transfer_through_t)););

    if(bArmReady())
        vWriteData2Stm((uint8*)pThHdr);
}
...
PRIVATE void vReportVehicleData(MAC_McpsIndData_s *psMcpsInd)
{
    ...
    app_header_t *pheader = (app_header_t *)(psFrame->au8Sdu);
    pheader->msgtype = APP_VEHICLE_CARD_RSSI;
    if(bArmReady())
        vWriteData2Stm((uint8*)pheader);
}
...
{
    //uint8 u8rssichannel;
	SaveParam_t u8rssichannel;
	
    if(!bAHI_FullFlashRead(PARAM_STORE_ADDR, sizeof(SaveParam_t),(uint8*)&u8rssichannel))
    {
        EDBG(i2c_vPrintf("Flash Read Fail\n"););
        return FALSE;
    }
	...
	u8rssichannel.u8Channel = u8Channel;
    if(bAHI_FullFlashProgram(PARAM_STORE_ADDR, sizeof(SaveParam_t),(uint8*)&u8rssichannel))
    {
