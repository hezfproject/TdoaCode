...
* DEFINES
**************************************************************************************************/
#define PHY_PAYLOAD_LEN		 		(127+11)
#define MAC_PAYLOAD_LEN             127
...
uint8  MAC_UTIL_BuildandSendDataPAN(const MacParam_t *param, uint8 *p, uint8 len, uint8 DeliverMode, uint16 uniDstAddr, uint8 txOption)
{
	...
	if(mac_Payload == NULL)
	{
			mac_Payload = osal_mem_alloc(MAC_PAYLOAD_LEN + 10); //addsome to avoid error
	}
	...
	if(len > MAC_PAYLOAD_LEN-totalen)   //truncate if too long 
	{
		len =  MAC_PAYLOAD_LEN-totalen;
	}
	...
	if(DeliverMode == MAC_UTIL_UNICAST)
	{
		...
		MAC_UTIL_McpsDataReq(mac_Payload, totalen, param->panID, DstAddr, txOption);//MAC_TXOPTION_ACK);
	}
	else
	{
		sAddr_t DstAddr;
		...
		MAC_UTIL_McpsDataReq(mac_Payload, totalen,param->panID, DstAddr, txOption);//MAC_TXOPTION_NO_RETRANS);
	}
	//osal_mem_free(mac_Payload);
	return totalen;
}
...
*/
void MAC_UTIL_McpsDataReq(const uint8* data, uint8 dataLength, uint16  panID, sAddr_t dstAddr,uint8 txOption)
{
	macMcpsDataReq_t  *pData;
	
#ifdef CHARGEED_PROJECT
...
#else
		pData->mac.msduHandle = 0;//handle++;
#endif

#ifdef CHARGEED_PROJECT
		pData->mac.txOptions = txOption;
#else
		pData->mac.txOptions = txOption |MAC_TXOPTION_NO_CNF;
#endif
