/**************************************************************************************************
Filename:       CR.c
Revised:        $Date: 2011/05/10 23:47:03 $
...
static void Station_start_timerEx(uint8 taskID, uint16 event_id, uint16 timeout_value);
...
static uint8 Station_ParseChargeCardFrame(const sAddr_t* sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi);
static uint8 Station_ParseGasMonitorFrame(const sAddr_t* sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi);
static uint8 Station_ParseMobileFrame(const sAddr_t* sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi);

static void Station_ProcessSPICB(const  SPIMSGPacket_t * rxmsg);
static uint8  Station_ParseFrameLocal(uint16 shortAddr, const uint8 *p, uint16 len);
/**************************************************************************************************
*
* @fn          Station_Init
*
...
void Station_Init(uint_8 taskId)
{
    ...
    // RegisterForSpi(Station_TaskId);

    Station_SpiBuf = (uint8*) osal_mem_alloc(MINEAPP_MAX_DATA_LEN + 10);
    Station_AirBuf = (uint8*) osal_mem_alloc(MINEAPP_MAX_DATA_LEN + 10);
    ...
    /* start up*/
    Station_start_timerEx(Station_TaskId, STATION_RFMAC_EVENT, 5);
}
...
uint16 Station_ProcessEvent(uint_8 taskId, uint16 events)
{
        ...
        while((pMsg = osal_msg_receive(Station_TaskId)) != NULL)
        {
            ...
            case MAC_MLME_START_CNF:
            {
                if(pData->startCnf.hdr.status == MAC_SUCCESS)
                {
                    ...
                    /* detect spi */
                    Station_start_timerEx(Station_TaskId, STATION_DETECT_BLAST_EVENT, 5);

                    /* report restart status */
                    Station_start_timerEx(Station_TaskId, STATION_STATUS_REPORT_EVENT, 10);
                }
                else
                {
                    Station_DevInfo.State = E_STATION_INIT;
    ...
    {
        Station_DevInfo.extAddr[0] = 0x07;
        Station_DevInfo.extAddr[1] = 0x2E;
        Station_DevInfo.extAddr[2] = 0xFF;
        Station_DevInfo.extAddr[3] = 0xFF;
        Station_DevInfo.extAddr[4] = 0xFF;
        Station_DevInfo.extAddr[5] = 0x03;
        Station_DevInfo.extAddr[6] = 0x19;
        Station_DevInfo.extAddr[7] = 0xFF;

        HAL_ASSERT(Station_DevInfo.extAddr[EXT_MACADDR_TYPE] == EXT_MACADDR_TYPE_SUBSTATION);
        HAL_ASSERT(Station_DevInfo.extAddr[EXT_MACADDR_CHANNEL] <= 0x1A && Station_DevInfo.extAddr[EXT_MACADDR_CHANNEL] >= 0x0B);
...
static void Station_ParseFrame(const macMcpsDataInd_t* pdataInd)
{
    ...
    case CARD_CLUSTERID:
    {
        Station_ParseCardFrame(&pdataInd->mac.srcAddr, pdataInd->mac.srcPanId, appdata, pdataInd->mac.rssi);
        break;
    }
    case CHARGEED_CLUSTERID:
    {
        Station_ParseChargeCardFrame(&pdataInd->mac.srcAddr, pdataInd->mac.srcPanId, appdata, pdataInd->mac.rssi);
        break;
    }
    case MINEAPP_CLUSTERID:
    {
        Station_ParseMobileFrame(&pdataInd->mac.srcAddr, pdataInd->mac.srcPanId, appdata, pdataInd->mac.rssi);
        break;
    }
    case GASMONITOR_CLUSTERID:
    {
        Station_ParseGasMonitorFrame(&pdataInd->mac.srcAddr, pdataInd->mac.srcPanId, appdata, pdataInd->mac.rssi);
        break;
    }
    }
}
static uint8 Station_ParseCardFrame(const sAddr_t * sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi)
{
    ...
    case SSREQ:
    {
        if(msdu.len != sizeof(app_ssReq_t)) return FAILURE;
        ...
        if(reqtype == SSREQ_POLL)
        {
            if(Station_DevInfo.retreat_flag == STATION_RETREAT_NORMAL)
            {
                ...
                MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *) &app_ssReq, sizeof(app_ssReq_t), MAC_UTIL_UNICAST,
                                             sAddr->addr.shortAddr, 0);
            }
            else if(Station_DevInfo.retreat_flag == STATION_RETREAT_RETREAT)
            {
                app_Urgent_t app_Urgent;
                app_Urgent.msgtype = URGENT;
                app_Urgent.urgenttype = RETREAT;
                app_Urgent.value = 0;
                MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *) &app_Urgent, sizeof(app_Urgent), MAC_UTIL_UNICAST,
                                             sAddr->addr.shortAddr, 0);
            }
            else if((Station_DevInfo.retreat_flag == STATION_RETREAT_CANCELRETREAT))
            {
                app_Urgent_t app_Urgent;
                app_Urgent.msgtype = URGENT;
        ...
        ret =  Station_SendDataToSpi(sAddr->addr.shortAddr, (uint8*)&ssInd, sizeof(ssInd));

        break;
    }
    case URGENT:
    {
        if(msdu.len != sizeof(app_Urgent_t)) return FAILURE;
        ...
        if(urgenttype == ALERT)
        {
            ...
            app_Urgent.value = 0;
            MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *) &app_Urgent, sizeof(app_Urgent_t), MAC_UTIL_UNICAST,
                                         sAddr->addr.shortAddr, 0);
        }
...
static uint8 Station_ParseChargeCardFrame(const sAddr_t * sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi)
{
    ...
    case CHARGEED_SSREQ:
    {
        if(msdu.len != sizeof(app_chargeed_ssReq_t)) return FAILURE;  // do not use locnode, so size is sizeof(app_chargeed_ssReq_t)
        ...
        if(reqtype == SSREQ_POLL)
        {
            ...
            MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *) &app_chargeed_ssRsp, sizeof(app_chargeed_ssRsp_t),
                                         MAC_UTIL_UNICAST, sAddr->addr.shortAddr , 0);
        ...
        ret =  Station_SendDataToSpi(sAddr->addr.shortAddr, (uint8*)&chargeed_ssInd, sizeof(chargeed_ssInd));
        break;
    }
    case URGENT:
    {
...
static uint8 Station_ParseGasMonitorFrame(const sAddr_t * sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi)
{
    ...
    {
        if(msdu.len != sizeof(app_GasAlarm_t))return FAILURE;
        ...
        MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *)&app_GasAlarm_ack, sizeof(app_GasAlarm_ack),
                                     MAC_UTIL_UNICAST, sAddr->addr.shortAddr, MAC_TXOPTION_ACK);
    }
...
static uint8 Station_ParseMobileFrame(const sAddr_t * sAddr, uint16 srcPanId, const sData_t msdu, int8 rssi)
{
    ...
    {
        if(msdu.len != sizeof(app_Timac_Mp_Scan_t)) return FAILURE;
        ...
        ret =  MAC_UTIL_BuildandSendDataPAN(&param, (uint8 *)&mp_scan, sizeof(mp_scan),
                                            MAC_UTIL_UNICAST, sAddr->addr.shortAddr, MAC_TXOPTION_ACK);
    }
    else
    {
        ...
        ret = Station_ParseFrameLocal(sAddr->addr.shortAddr, msdu.p, msdu.len);
    }
...
static void Station_ProcessSPICB(const  SPIMSGPacket_t* rxmsg)
{
    if(rxmsg == NULL || (rxmsg->DataLength + sizeof(SPIMSGPacket_t)) > MINEAPP_MAX_DATA_LEN)
    {
        return;
    }
    ...
    if(rxmsg->spihdr.dstAddr.addr.shortAddr != 0)
    {
        ...
        MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)(rxmsg + 1), rxmsg->DataLength,
                                     MAC_UTIL_UNICAST, rxmsg->spihdr.dstAddr.addr.shortAddr, MAC_TXOPTION_ACK);
    }
    else
    {
        ...
        case CROSSPAN:
        {
            if(rxmsg->DataLength!=(sizeof(app_CrossPan_t)+appwrapper->app_CrossPan.len))
            {
                return;
            }
        ...
        case GASDEV_SMS:
        {
            if(rxmsg->DataLength!=(sizeof(app_gassms_t) -1 + appwrapper->app_gassms.len))
            {
                return;
            }
            ...
            MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)(rxmsg + 1), rxmsg->DataLength,
                                         MAC_UTIL_UNICAST, 0 , MAC_TXOPTION_ACK);
            break;
        }
        default:
        {
...
static uint8  Station_ParseFrameLocal(uint16 shortAddr, const uint8 *p, uint16 len)
{
    APPWrapper_t* AppPkt = (APPWrapper_t*)(p);
    ...
    case ZB_CMD_UP:
    {
        app_CmdUp_t *p = (app_CmdUp_t *)(AppPkt);

        app_CmdDown_t app_CmdDown;

        app_CmdDown.msgtype = ZC_CMD_DOWN;

        if(p->cmd == CMD_UP_DIALUP)
        {
            app_CmdDown.cmd = CMD_DOWN_CALL;
        }
        else if(p->cmd == CMD_UP_FOUND)
        {
            app_CmdDown.cmd = CMD_DOWN_FOUND;
        }
        else if(p->cmd == CMD_UP_BUSY)
        {
            app_CmdDown.cmd = CMD_DOWN_BUSY;
        }
        else if(p->cmd == CMD_UP_ACCEPT)
        {
            app_CmdDown.cmd = CMD_DOWN_ACCEPT;
        }
        else if(p->cmd == CMD_UP_CLOSE)
        {
            app_CmdDown.cmd = CMD_DOWN_CLOSE;
        }

        app_CmdDown.srcnbr = p->srcnbr;
        app_CmdDown.dstnbr = p->dstnbr;

        uint16 dstAddr = strtoul(p->dstnbr.Nmbr,NULL, 10);

        MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)&app_CmdDown, sizeof(app_CmdDown),
                                     MAC_UTIL_UNICAST, dstAddr, MAC_TXOPTION_ACK);

        break;
    }
    case TIMAC_JOIN_NOTIFY:
    {
        app_Timac_JoinNwk_t *p = (app_Timac_JoinNwk_t *)(AppPkt);

        app_Timac_JoinNwk_t app_Timac_JoinNwk;
        app_Timac_JoinNwk.msgtype = TIMAC_JOIN_NOTIFY;
        app_Timac_JoinNwk.joinnwktype = APP_TIMAC_JOINNWK_SUCCESS;
        app_Timac_JoinNwk.seqnum  = p->seqnum;
        app_Timac_JoinNwk.panid = p->panid;
        app_Timac_JoinNwk.status = p->status;
        app_Timac_JoinNwk.srcnbr = p->srcnbr;

        MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)&app_Timac_JoinNwk, sizeof(app_Timac_JoinNwk),
                                     MAC_UTIL_UNICAST, shortAddr, MAC_TXOPTION_ACK);

        break;
    }
    case VOICE:
    {
        app_Voice_t *p = (app_Voice_t *)(AppPkt);

        app_Voice_t *pVoice = (app_Voice_t *)Station_AirBuf;
        pVoice->msgtype = VOICE;
        pVoice->len = p->len;
        pVoice->blk = p->blk;
        pVoice->srcnbr = p->srcnbr;
        pVoice->dstnbr = p->dstnbr;

        memcpy((uint8*)(pVoice+1), (uint8*)(p+1), p->len);

        uint16 dstAddr = p->dstnbr;

        MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)pVoice, sizeof(app_Voice_t)+pVoice->len,
                                     MAC_UTIL_UNICAST, dstAddr, MAC_TXOPTION_ACK);
    ...
    case MP_POLL_REQ:
    {
        app_MP_Poll_Req_t *p = (app_MP_Poll_Req_t *)(AppPkt);

        app_MP_Poll_Ack_t  app_MP_Poll_Ack;

        app_MP_Poll_Ack.msgtype = MP_POLL_ACK;
        app_MP_Poll_Ack.nbr = p->nbr;
        app_MP_Poll_Ack.flag = MP_POLL_FLAG_START;

        MAC_UTIL_BuildandSendDataPAN(&macparam, (uint8 *)&app_MP_Poll_Ack, sizeof(app_MP_Poll_Ack),
                                     MAC_UTIL_UNICAST, shortAddr, MAC_TXOPTION_ACK);
