...
void TDOARxGoodCallback(const dwt_callback_data_t *pstRxCallBackData)
{
    ...
    if ((pstInstMsg->stRxMsgFromDw.u8MessageData[0] == RTLS_TDOA_BLINK_SEND ) || 
        (pstInstMsg->stRxMsgFromDw.u8MessageData[0] == 102 ))
    {
        PrintfUtil_vPrintf("%d -> %d %d %d \n",u16SrcAddr, u16DestAddr, u16Seqnum, pstInstMsg->stRxMsgFromDw.u8MessageData[0]);
    }
...
//#ifdef	 PRINTF_vReportCardDistance_SendData
    ...
    for (i = 0; i < pstBetterStdCardTable->u8CardCount; i++)
    {
        ...
        {
            stAppTdoaData.stAppTdoaMsg[i].i8Rssi = pstBetterStdCardTable->stBetterTable[i].i8Rssi;
            break;
        }
    }
    ...
    for (i = 0; i < u16TestTagNum; i++)
    {
        ...
        u16TestTagNum,
		stAppTdoaData.stAppTdoaMsg[i].u16StationID,
		stAppTdoaData.stAppTdoaMsg[i].u16TestCardID,
		stAppTdoaData.stAppTdoaMsg[i].u16StandardCardID,
		stAppTdoaData.stAppTdoaMsg[i].u8Status,
		stAppTdoaData.stAppTdoaMsg[i].i8Rssi,
		stAppTdoaData.stAppTdoaMsg[i].u16Cardseqnum);
    }
