...
typedef void (__near_func *halSPICBack_t) (void*);
...
typedef struct
{
  uint16 		   maxBufSize; 
  bool                intEnable;
  //halSPICBack_t      callBackFunc;
  //halSPICBack_t      callBackFunc2;
}halSPICfg_t;
...
#define DMA_RXBUF   0xDFF9
#define DMA_TXBUF   0xDFC1
 ...
 */
extern __near_func uint8 HalSPIRead (uint8 **pBuffer, uint16 length );

/*
 * Write a buff to the SPI
 */
extern __near_func uint8 HalSPIWrite (uint8 *pBuffer, uint16 length );

/*
 *Unblock the rx from App.
 */
//extern __near_func uint8 HalSPIUnblock(void);
...
/*
 * Register the task for spi events.
 */
uint8 RegisterForSpi( uint8 task_id );
