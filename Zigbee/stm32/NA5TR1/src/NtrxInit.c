...
{
	if( NtrxReadReg( NA_Version_O ) != NA_Version_I )
		return False ;
	if( NtrxReadReg( NA_Revision_O ) != NA_Revision_I )
		return False ;
	return True ;
}
...
//-----------------------------------------------------------------------------
    ...
    #ifdef FAKE_CALIBRATION
    {
        extern void NtrxAllFakeCalibration(void);
        static Bool bFirstBoot = True;

        if (!bFirstBoot)
        {
            NtrxAllFakeCalibration();
        }
        else
        {
            bFirstBoot = False;
		...
		NtrxSetField( NA_UseFec, True) ;//NA_GateSizeFramesync_O
		NtrxRestart() ;
       // NtrxAllCalibration() ;
	}
 ...
 */
/**************************************************************************/
void NTRXSetTestCarrierMode (Bool value)
{
	static uint8 fdma;
    #define SHADOW_REG_COUNT        128

extern    NtrxBufferType NtrxShadowReg[SHADOW_REG_COUNT] ;

	NtrxWriteReg (NA_RxCmdStop_O, NtrxShadowReg[NA_RxCmdStop_O] | (1 << NA_RxCmdStop_B));
	NtrxWriteReg (NA_RxIntsReset_O, 0x7f);

	if (value == True)
	{
		fdma = NtrxShadowReg[NA_FdmaEnable_O];
		NtrxShadowReg[NA_FdmaEnable_O] &= ~(1 << NA_FdmaEnable_B);
		NtrxWriteReg (NA_FdmaEnable_O, NtrxShadowReg[NA_FdmaEnable_O]);

		NtrxShadowReg[NA_EnableLO_O] |= ((1 << NA_EnableLO_B) | (1 << NA_EnableLOdiv10_B));
		NtrxWriteReg (NA_EnableLO_O, NtrxShadowReg[NA_EnableLO_O]);

		NtrxShadowReg[NA_CsqSetValue_O] = 0x1F;
		NtrxWriteReg (NA_CsqSetValue_O, NtrxShadowReg[NA_CsqSetValue_O]);
		...
		NtrxWriteReg (NA_EnableTx_O, NtrxShadowReg[NA_EnableTx_O]);

		NtrxShadowReg[NA_EnableExtPA_O] |= 0x20;
		NtrxWriteReg (NA_EnableExtPA_O, NtrxShadowReg[NA_EnableExtPA_O]);
	}
	else
	{
		NtrxShadowReg[NA_EnableExtPA_O] &= ~(0x20);
