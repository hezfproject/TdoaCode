...
uint16  save_locatedistance=32767;
...
uint16  GasMonitor_Get_Rate(void)
{
    return Rate;
}
int8 GasMonitor_Get_RSSI(void)
{
    return RSSI;
}
...
uint16 GasMonitor_GetLocateDistance(void)
{
    ...
    if(save_locatedistance == 32767 || save_locatedistance == 32766)
  	{
...
void	 GasMonitor_stationRSSI(uint8* pNum10000,uint8* pNum1000,uint8* pNum100, uint8* pNum10,uint8* pNum1,int8 bRSSI)
{
    if(bRSSI < 0)
        bRSSI = -bRSSI;
    *pNum10000= bRSSI/10000;
    bRSSI -= (*pNum10000)*10000;
    *pNum10000 = (*pNum10000)%10;
