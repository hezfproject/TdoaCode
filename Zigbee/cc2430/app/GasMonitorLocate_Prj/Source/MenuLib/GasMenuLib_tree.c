...
void menu_locate_display()
{
    ...
    if(Locate_isvalid&&(GasMonitor_GetLocateDistance()<1000))
    {
        ...
        LCD_Str_Print((uint8 *)"成功率:", 0, 3,TRUE);
		//uint16 GasMonitor_Get_SAVE_LocateDistance(void)
        GasMonitor_stationID_fromdensity(&n1,&n2,&n3,&n4, &n5, GasMonitor_Get_Rate());	
        //menu_LocatePrint(n3,n4,n5,true);
        a[0]=n1+'0';
        a[1]=n2+'0';
        a[2]=n3+'0';
        ...
        a[4]=n5+'0';
        a[5]='\0';
        LCD_Str_Print((uint8 *)(a+3), 7, 3,TRUE); 
        LCD_Str_Print("/20", 9, 3,TRUE);
        LCD_Str_Print((uint8 *)"信号:", 0, 2,TRUE);
        if(GasMonitor_Get_RSSI()<0)
        {
            LCD_Str_Print((uint8 *)"-", 7, 2,TRUE);
        }
        GasMonitor_stationRSSI(&n1,&n2,&n3,&n4, &n5, GasMonitor_Get_RSSI());
        a[0]=n1+'0';
        a[1]=n2+'0';
        a[2]=n3+'0';
        a[3]=n4+'0';
        a[4]=n5+'0';
        a[5]='\0';
        LCD_Str_Print((uint8 *)(a+3), 8, 2,TRUE);
