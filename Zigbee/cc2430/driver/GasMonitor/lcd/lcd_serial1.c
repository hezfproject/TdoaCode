...
#define LCD_CS_BIT       2
#define LCD_SDA_BIT     3
#define LCD_SCK_BIT     4
...
uint8 LcdReadReg(uint8 addr)
{
	...
	P0DIR &= ~(0x01<<LCD_SDA_BIT);	// set SDA  to input
	...
	P0DIR |= (0x01<<LCD_SDA_BIT);	// set SDA back to output
	return data;
}
/****************************************************************
*** Write lcd memory
...
void InitialLcd(void)
{
	...
	//LcdWriteReg(0x10,0xf2);//0xff);	//1/9Bias  最大对比度
	//DelayMs(5);
	//LcdWriteReg(0x12,0xfc);		//4x,最小电流驱动
        LcdWriteReg(0x10, LCD_PARAMETER1);
	DelayMs(5);
        LcdWriteReg(0x12, LCD_PARAMETER2);  
        LcdWriteReg(BLTR,0x00);	
...
void LCDIntoSleep()
{
    ...
    LCD_CS = 1;
    LCD_SCK = 0;   
    P0DIR &= ~(0x01<<LCD_SDA_BIT);    // LCD_SDA  input
    return;
}
void LCDWakeUp()
{
