...
rt_err_t ping(char* target, rt_uint32_t time, rt_size_t size)
{
	...
	if(if_sockt> (-1))
		lwip_close(if_sockt);
	if ((if_sockt = lwip_socket(AF_INET, SOCK_RAW, IP_PROTO_ICMP)) < 0)
	{
			rt_kprintf("create socket failled\n");
		return -RT_ERROR;
	}
	lwip_setsockopt(if_sockt, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));

	while (1)
	{
		if (ping_send(if_sockt, &ping_target) == ERR_OK)
		{
