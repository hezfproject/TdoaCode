...
void CNumProgrammerDlg::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Text(pDX, IDC_EDIT5, m_CustomerCode);
...
BOOL CNumProgrammerDlg::OnInitDialog()
{
    ...
    m_ProductType.InsertString(0,CString("定位卡"));
    m_ProductType.InsertString(1,CString("一代手机"));
    ...
    ((CEdit*)GetDlgItem(IDC_EDIT5))->SetLimitText(2);
...
LRESULT CNumProgrammerDlg::OnDevListMessage(WPARAM wParam, LPARAM lParam)
{
...
void CNumProgrammerDlg::IEEEAddressNum2EachValue()
{
    ...
    m_Channel = m_IEEEAddr[1];

    unsigned int producttypeidx = m_IEEEAddr[2]-1;
    if (producttypeidx < m_ProductType.GetCount())
    {
    ...
    m_Version = m_IEEEAddr[3];

    m_CustomerCode.Empty();
    m_CustomerCode += m_IEEEAddr[4];
    m_CustomerCode += m_IEEEAddr[5];
    m_Number = (((unsigned int)m_IEEEAddr[6])*256 + (unsigned int)m_IEEEAddr[7]);
    UpdateData(false);
    return;
}
...
bool CNumProgrammerDlg::IEEEAddressEachValue2Num()
{
    ...
    m_IEEEAddr[3] = m_Version;
    m_IEEEAddr[4] = (unsigned char)m_CustomerCode[0];
    m_IEEEAddr[5] = (unsigned char)m_CustomerCode[1];

    m_IEEEAddr[6] = (m_Number>>8) & 0x000000FF;
    m_IEEEAddr[7] = (m_Number) & 0x000000FF;
