...
{
#if (HAL_ADC == TRUE)
    APCFG |= (1 << 6); // P0_6 enable analog channel
    //P0SEL |= (1 << 6);
    P0DIR &= ~(1 << 6);
    P0INP &= ~(1 << 6);
    P2INP &= ~(1 << 5);
...
uint16 HalAdcRead2( uint8 channel, uint8 resolution, uint8 reference)
{
    int16  reading = 0;
    uint8  resbits;
    uint8  adcChannel = 1 << 6;
    ...
    APCFG |= adcChannel; // enable analog channel

    /* writing to this register starts the extra conversion */
    ADCCON3 = channel | resbits | reference;

    /* Wait for the conversion to be done */
    while (!(ADCCON1 & HAL_ADC_EOC));

    APCFG &= ~adcChannel; // P0_6 enable analog channel
