/**************************************************************************************************
Filename:       CR.c
Revised:        $Date: 2011/11/09 14:39:11 $
...
*                                           Macros
**************************************************************************************************/
#define STATION_PARAM_ADDR                     0xFEF0  //param space 1EF0-1EFF
...
static uint32 u32Data2Arm[30];
static struct nwkhdr* pnwk_data = (struct nwkhdr*)u32Data2Arm;
...
#define  APP_PROTOCOL_TYPE_CARD   3
  #define  TOF_CARD_RSSI            5
  #define  TOF_CARD_ALARM           7
  #define TOF_CARD_CHECKIN          4
#define APP_TOF_MSG_NEW_ALARM       14

typedef struct
{
    app_header_t app_tof_head;

    uint16 u16ShortAddr;    // card or locator's addr
    uint8 u8Status;            // app_tof_alarm_type_te
    //uint8 u8Reserved;
    uint8  u8ExciterID;
} app_tof_alarm_ts;
...
static void vConvertEndian16(uint16* pu16Data)
{
    SysUtil_vConvertEndian(pu16Data, sizeof(uint16));
}


static void vWriteData2Stm(uint8* pbuf,uint16 srcAddr)
{
    ...
    pnwk_data->dst = 0xFFFF;

    pnwk_data->src = Station_DevInfo.PanId;
    //vConvertEndian16(&(pnwk_data->src));

    //vConvertEndian16(&(pnwk_data->dst));
    //vConvertEndian16(&(pnwk_data->len));
    //vConvertEndian16(&(pAppHead->len));
    osal_memcpy(pnwk_data+1, pbuf, u16App_len + sizeof(app_header_t));

    Station_SendDataToSpi(srcAddr,(uint8 *) pnwk_data,nwk_len);
}
...
static void vReportCardExcite(uint16 u16ShortAddr, uint8 u8CardStatus,uint8 u8ExciteID)
{
    ...
    vWriteData2Stm((uint8*)&app_tof_alarm_data,u16ShortAddr);
}
...
static void Station_ParseFrame(const macMcpsDataInd_t* pdataInd)
{
    if ((pdataInd == NULL)
            ...
            ||(pdataInd->msdu.len  <= sizeof(rf_tof_card_data_ts)-1)
    ...
    rf_tof_card_data_ts*  tof_card_data = (rf_tof_card_data_ts*) (pdataInd->msdu.p);
    if(tof_card_data->tof_head.protocoltype == APP_PROTOCOL_TYPE_CARD)
    {
        if(tof_card_data->tof_head.msgtype ==TOF_CARD_RSSI)
        {
        ...
        else if(tof_card_data->tof_head.msgtype == TOF_CARD_ALARM)
        {
            vReportCardExcite(pdataInd->mac.srcAddr.addr.shortAddr,\
                tof_card_data->u8CardStatus,tof_card_data->u8Reserved);
            if(tof_card_data->u8CardStatus & 0x80)
            {
                #if 0
                MAC_UTIL_BuildandSendDataPAN(&param,
        ...
        else if(tof_card_data->tof_head.msgtype == TOF_CARD_CHECKIN)
        {
            vReportCardExcite(pdataInd->mac.srcAddr.addr.shortAddr,0x80,0);
    ...
    //;1Generation: ZigBee card
            ...
            case BLAST_CLUSTERID:
            {
                ...
                pstBlast->rssi = pdataInd->mac.rssi;
...
//; 2G TIMAC card
    ...
    {
        if (msdu.len != sizeof(rf_tof_card_data_ts))
            return FAILURE;

        rf_tof_card_data_ts*  tof_card_data = (rf_tof_card_data_ts*) (msdu.p);

        if(tof_card_data->u8CardStatus &0x06)//alert  or  no power
        {
                 app_Urgent_t  app_Urgent;
                 app_Urgent.msgtype = URGENT;
                 app_Urgent.value =(uint8)(tof_card_data->u16Battery&0xff);

                 if(tof_card_data->u8CardStatus & 0x06 == 0x06)//alert  or  no power
                 {
                        app_Urgent.urgenttype = NOPWR;
                        ret = Station_SendDataToSpi(sAddr->addr.shortAddr, (uint8 *)&app_Urgent, sizeof(app_Urgent_t));
                 }

                 if(tof_card_data->u8CardStatus & 0x06 == 0x04) //no power
                 {
                        app_Urgent.urgenttype = NOPWR;
                 }
                 else
        ...
        else
        {
                app_chargeed_ssInd_t chargeed_ssInd;
                chargeed_ssInd.rssipkt.rssi = rssi;
                chargeed_ssInd.rssipkt.NWK_ADDR = 0xFFFF;
                chargeed_ssInd.rssipkt.NODE_ADDR = sAddr->addr.shortAddr;
                chargeed_ssInd.rssipkt.seqnum = tof_card_data->u16SeqNum;
                chargeed_ssInd.msgtype = CHARGEED_SSIND;
                chargeed_ssInd.LocCnt = 0;
