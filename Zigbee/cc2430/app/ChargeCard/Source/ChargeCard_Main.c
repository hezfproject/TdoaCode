...
void ChargeCard_Main_TimerCallBack(uint8 timerId, uint8 channel, uint8 channelMode);

/* This callback is triggered when a key is pressed */
void ChargeCard_Main_KeyCallback(uint16 keys, uint8 state);
...
int main(void)
{
  ...
  HalTimerConfig ( OSAL_TIMER,                         // 8bit timer3
                   ...
                   FALSE,                              // Use interrupt
                   ChargeCard_Main_TimerCallBack);        // Channel Mode

  /* Setup Keyboard callback */
  HalKeyConfig(ChargeCard_KEY_INT_ENABLED, ChargeCard_Main_KeyCallback);
...
void ChargeCard_Main_KeyCallback(uint16 keys, uint8 state)
{
  if ( ChargeCard_TaskId != TASK_NO_TASK )
  {
    ChargeCard_HandleKeys (keys, state);
  }
}

/**************************************************************************************************
 *
 * @fn      ChargeCard_PowerMgr
 *
 ...
 *
 **************************************************************************************************/
void ChargeCard_PowerMgr(uint8 enable)
{
