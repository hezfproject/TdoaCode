    ...
    if(pPayload.p == NULL || pPayload.len == 0)
{
    ...
    case REPORT_STATION:
    {
        ...
        if(pPayload.len == sizeof(app_Report_t)
                && panid == MP_DevInfo.CoordPanID
                && (p->dstnbr==(((MP_DevInfo.ExitAddr[EXT_MACADDR_DEVID_HBYTE] <<8)&0xff00)|(MP_DevInfo.ExitAddr[EXT_MACADDR_DEVID_LBYTE]&0x00ff))))
        {
            MP_ParseVersion(p);
        }
        break;
...
bool Is_A_New_SMS(app_SMS_t *p)
{
        bool flag = TRUE;
        uint8 quantity = 0;
        uint16  offset,Seqnum;
        uint8 numbuf[NMBRDIGIT+1];
        
 #ifdef NEW_DOUBLE_NVID_OP
       
        quantity = Get_SMS_Quantity();
        if(!quantity) return flag;

        for(uint8 i=0; i<quantity;i++)
        {
                offset = sizeof(uint8) + i * SMS_NV_LEN;
                menu_doubleNVID_read(SMS_NVID, offset , sizeof(uint16), &Seqnum);
                memset(numbuf, 0, NMBRDIGIT+1);
                menu_doubleNVID_read(SMS_NVID, offset+sizeof(uint16)+SMS_LEN_TYPE_SIZE, NMBRDIGIT, numbuf);

                 if((p->blk==Seqnum)&&(strcmp((char *)&p->nmbr, (char *)numbuf) == 0))
                {
                    flag=FALSE;
                    return flag;
                }
...
void MP_ParseRejoin(app_RejoinNwk_t *p)
{
 ...
 void MP_ParseVersion(app_Report_t *p)
 {
        if(p->len<=VERSION_MAX_CHARS)
            {
                osal_memcpy(version, p->version, p->len);
                version[p->len]=='\0';
                Version_isGet=TRUE;
            }
 }
