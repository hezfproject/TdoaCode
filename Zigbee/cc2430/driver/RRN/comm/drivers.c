/**************************************************************************************************
Filename:       drivers.c
Revised:        $Date: 2011/03/15 18:37:24 $
...
uint16 Hal_ProcessEvent( uint8 task_id, uint16 events )
{
	...
	if (events & HAL_BEEPER_EVT)
	{
		...
		if(ZSuccess != osal_start_timerEx(Hal_TaskID, HAL_BEEPER_EVT, Hal_BeeperFreq*30))
		{
			SystemReset();
		}
...
void HalStopBeeper(uint8 type, bool force)
{
	...
	if((Hal_IsBeeping && Hal_BeeperType==type) || force)
	{
		...
		osal_unset_event(Hal_TaskID, HAL_BEEPER_EVT);
		beep_stop();
		Hal_IsBeeping = false;
	}
}
