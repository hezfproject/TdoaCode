/**************************************************************************************************
  ...
  Revised:        $Date: 2011/09/30 02:34:19 $
...
#else
        cfg->flag = 0;
        HAL_UART_ASSERT((config->rx.maxBufSize < 280));
...
uint16 HalUARTRead(uint8 port, uint8* buf, uint16 len)
{
    ...
    /* if the buffer have round over*/
    if (cfg->rx_count >= cfg->rxMax)
    {
        if (cfg->rxHead == cfg->rxMax - 1)
        {
            cfg->rxTail = 0;
        }
        else
        {
            cfg->rxTail = cfg->rxHead + 1;
        }
    }
    ...
    /* flush rx buffer after uart read */
    cfg->rxHead = 0;
    cfg->rxTail = 0;
    cfg->rx_count = 0;
