...
#define RANGING_PERIOD          1000				// 测距时间间隔
...
//	#endif
	...
	printf("R - Reset Address\n");
	printf("I - Increase Address\n");
	printf("D - Decrease Address\n");
}
...
void TagRangingProcess(void)
{
	...
	for( i = 0 ; i < MAX_NEIGHBORS_NUM ; i ++ ){
		if(devInfo.neighbors[i].address == NT_INVALID_ADDRESS){
			retval[i] = False ;
		}else{extern float GetDistance( void );
            ...
            dist = GetDistance() ;
            printf("Distance of %02X %02X %02X %02X %02X %02X = %f\n",
                DestDev[0],DestDev[1],DestDev[2],DestDev[3],DestDev[4],
                DestDev[5],dist);
            if(retval[i]){
				cnt++;
	...
	if(!frm) 
	{
		printf("Ranging frame allocated fail.");
		return;
	}
    ...
    #if 0
{
    ...
    uint8 *p = (uint8 *)(frm);
    printf("\nRanging record:\n");
    for(i=0; i<sizeof(frm->hdr)+(frm->numrec)*sizeof(NT_ranging_record_t)+5; i++)
        printf("0x%x ", p[i]);
    printf("\n");
}
    #endif
...
//	printf("tag power saving on.\n");
//	osal_pwrmgr_device(PWRMGR_BATTERY); //允许进入休眠_
//	#else
//	osal_pwrmgr_device(PWRMGR_ALWAYS_ON);   //一直开启，不进入休眠_
//	#endif
	
	osal_pwrmgr_device(PWRMGR_BATTERY);   //一直开启，不进入休眠_
	osal_pwrmgr_task_state(App_TaskId, PWRMGR_CONSERVE);    //申明该任务是否要节能	  PWRMGR_CONSERVE
...
//		#if (ANCHOR || TAG)
		case FT_BEACON:
			AppProcessBeacon(pMsg); //根据beacon更新邻居表
			break;
//		#endif
...
//	#if TAG
	...
	if(devInfo.ChkSrchNbr){
		...
		if(lifetime >= TAG_SEARCH_NEIGHBOR_TIMEOUT){
			...
			TagUpdateParent();  //更新父节点。DEMO1/2以广播地址为父节点。
...
//{
//	NT_beacon_t bcn;
//	AppBuildFrmHdr(&bcn.hdr, FT_BEACON, 0, 0, anchor_bsn++, devInfo.address, NT_BROADCAST_ADDRESS, 1, sizeof(NT_beacon_t) - sizeof(NT_frm_hdr_t));
...
void TagSendBeacon(void)
{
    ...
    i = sizeof(NT_beacon_t);
    j = sizeof(NT_frm_hdr_t);
	AppBuildFrmHdr(&bcn.hdr, FT_BEACON, 0, 0, tag_bsn++,devInfo.address, 
        NT_BROADCAST_ADDRESS, 1,  i-j );
	bcn.pathok = (devInfo.parent != NT_INVALID_ADDRESS ? 1 : 0);

// 	printf("Send a beacon, length %d, time %s:\n   ", bcn.hdr.len, CSTR( __TIME__ ));
// 	
// 	for(i = 0; i < (&bcn.hdr)->len; i++)
// 	{
// 		printf("%02X ", bcn.[i]);
// 	}
// 	printf("\n");	
...
void AppProcessBeacon(MacMsg_t * pMsg)
{
	NT_beacon_t *frm = (NT_beacon_t*)pMsg->payload;
...
//	#endif
	...
	if(events & APP_EVENT_TICK) {   //定时时间，每5秒产生一次
		devInfo.ticks++;
//		#if STATION
