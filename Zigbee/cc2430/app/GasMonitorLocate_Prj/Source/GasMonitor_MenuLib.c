...
void menu_LocatePrint(uint8 n1,uint8 n2, uint8 n3, bool forcePrint)	
{
	...
	if(forcePrint)
	{
		...
		x = (GASDISP_START_X+1)+6;
		y = GASDISP_START_Y+1-1;
		LCD_Clear(x,  y, x+1, y);
		if(n1!=0)
		LCD_BigCharPrint(n1+'0', x, y*LCD_LINE_HIGH);

		x = (GASDISP_START_X+1)+2+6;
		y = GASDISP_START_Y+1-1;
		LCD_Clear(x, y, x+1, y);
		if(n1!=0 || n2!=0)
		LCD_BigCharPrint(n2+'0', x, y*LCD_LINE_HIGH);

		x = (GASDISP_START_X+1)+4+6;
		y = GASDISP_START_Y+1-1;
		LCD_Clear(x, y, x+1, y);
