...
PRIVATE void vProcessAppEvent()
{
    ...
    {
        PrintfUtil_vPrintf("T\n");
        vTransmitDataPacket(au8Data, 80, 0x0001);
        EventUtil_vUnsetEvent(event);
...
PRIVATE void vHandleMcpsDataInd(MAC_McpsDcfmInd_s *psMcpsInd)
{
    ...
    //PrintfUtil_vPrintMem(psMcpsInd->uParam.sIndData.sFrame.au8Sdu, 16);

    uint32 *pPremble = (uint32*)psMcpsInd->uParam.sIndData.sFrame.au8Sdu;

    if(*pPremble == 0x36452738)
    {

        int8 dbm = SysUtil_vConvertLQI2Dbm(psMcpsInd->uParam.sIndData.sFrame.u8LinkQuality);

        uint32 count = *(pPremble+1);

        static uint32 recv=0,err=0;
        ...
        {
            err += count-last_count-1;
        }
        rate =(uint32) ((float)err*1000)/((float)(recv+err));
        last_count = count;

        PrintfUtil_vPrintf("rssi %i, err/recv: %d/%d, err rate:%d.%d%%\n ",dbm,err,recv,rate/10,rate-10*(rate/10));
...
PRIVATE void vStartCoordinator(void)
{
    ...
    sMlmeReqRsp.uParam.sReqStart.u8Channel = 0x10;
