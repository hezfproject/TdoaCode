...
#ifdef MENU_RF_DEBUG
static char*  const __code  ItemList_Settings[] = {RINGSETTING_CHINA,TIMESETTING_CHINA,BACKLIGHTSETTING_CHINA,PADLOCKSETTING_CHINA,RESTOREDEFAULT_CHINA,SET_CHANNEL_CHINA, SET_PANID_CHINA};
#else
...
static void    menu_contactlist_display(void)
{
...
#else
	osal_nv_read(MINEAPP_NV_CONTACT, 0, 1, &offset);
#endif
	...
	else if(node.ChildNum <= SCREEN_LINES)
	{
		...
		{
#ifdef NEW_DOUBLE_NVID_OP
			//menu_doubleNVID_read(CONTACT_NVID, sizeof(Contact_Node)*i+NMBRDIGIT+1, MAX_NAME_LEN, c_node.name);
			menu_Contact_ReadContact(&c_node, i);
#else
			osal_nv_read(MINEAPP_NV_CONTACT, sizeof(Contact_Node)*i+NMBRDIGIT+1, MAX_NAME_LEN, c_node.name);
#endif
	...
	else
	{
		...
		{
#ifdef NEW_DOUBLE_NVID_OP
			//menu_doubleNVID_read(CONTACT_NVID, sizeof(Contact_Node)*(node_info.show_item+i)+NMBRDIGIT+1, MAX_NAME_LEN, c_node.name);
			menu_Contact_ReadContact(&c_node, node_info.show_item+i);
#else
			osal_nv_read(MINEAPP_NV_CONTACT, sizeof(Contact_Node)*(node_info.show_item+i)+NMBRDIGIT+1, MAX_NAME_LEN, c_node.name);
#endif

			//osal_nv_read(MINEAPP_NV_CONTACT, sizeof(Contact_Node)*(node_info.show_item+i)+NMBRDIGIT+1, MAX_NAME_LEN, c_node.name);
			//name = (char*)c_node.name;
			//len = osal_strlen((uint8 *)name);
			LCD_Str_Print((uint8 *)c_node.name, 0, i+1, TRUE);
			//LCD_Memory_Print((uint8 *)name, len, 0, i+1);
		}
	}
...
{	
	...
	{     
		...
		{
#ifdef NEW_DOUBLE_NVID_OP
			//menu_doubleNVID_read(CONTACT_NVID, sizeof(Contact_Node)*node_info.sel_item+1, NMBRDIGIT, incoming_buf.p);
			Contact_Node contact_node;
			menu_Contact_ReadContact(&contact_node,node_info.sel_item);
			if(strlen((char*)contact_node.num) < NMBRDIGIT)
			{
				strcpy((char*)incoming_buf.p,(char*)contact_node.num);
			}
#else
			osal_nv_read(MINEAPP_NV_CONTACT, sizeof(Contact_Node)*node_info.sel_item+1, NMBRDIGIT, incoming_buf.p);
#endif
			incoming_buf.len = osal_strlen((char*)incoming_buf.p);
...
static void    menu_call_record_onkey(uint8 keys, uint8 status)
{
	...
	case HAL_KEY_CALL:
		...
		{
			osal_nv_read(NV_ID, (node_info.sel_item-1)*sizeof(Record)+1, NMBRDIGIT, incoming_buf.p);
			incoming_buf.len = osal_strlen((char*)incoming_buf.p);
			incoming_buf.p[incoming_buf.len] = '\0';
			menu_Dial(incoming_buf);
		}
...
#else
	osal_nv_read(MINEAPP_NV_SMS, 0, 1, &offset);
#endif
...
#ifdef NEW_DOUBLE_NVID_OP
			menu_doubleNVID_read(SMS_NVID, tmp, NMBRDIGIT,  num_buf.p);
#else
			osal_nv_read(MINEAPP_NV_SMS, tmp, NMBRDIGIT, num_buf.p);
#endif
...
#else
			osal_nv_read(MINEAPP_NV_SMS, tmp, 1, &SMS_flag);
#endif
...
#else
	osal_nv_read(MINEAPP_NV_SMS, 0, 1, &len);
#endif
...
static void    menu_shortmessage_writing_display(void)
{
	if(NULL == data_buf.p)
	{
		...
		if(NULL == Buffer_Init(&data_buf, SMS_MAX_LEN+1))
			return;
	}
...
#ifdef NEW_DOUBLE_NVID_OP
					menu_doubleNVID_read(SMS_NVID, offset, NMBRDIGIT, num_buf.p);
#else
					osal_nv_read(MINEAPP_NV_SMS, offset, NMBRDIGIT, num_buf.p);
#endif
					SET_ON_SM_SENDING();
...
static void menu_shortmessage_writing_onkey(uint8 keys, uint8 status)
{
	...
	if(input_status == OUTPUT_STATUS)
	{
		...
		if(menu_ChineseOutput_Length() > 0)
		{
			...
			if((data_buf.len >= SMS_MAX_LEN-1) || ((data_buf.len >= SMS_MAX_LEN-2)&&(output_p[0]>0x80)))
			{
...
void menu_sms_edit_handle_onkey(uint8 keys, uint8 status)
{
	if(keys == HAL_KEY_SELECT)
	{
		...
		else if(node_info.sel_item == 1)//save and send
		{
			...
			{
				osal_nv_write(MINEAPP_NV_SMS_SENDBOX, 1+SMS_MAX_LEN*offset, 1, &data_buf.len);
				osal_nv_write(MINEAPP_NV_SMS_SENDBOX, 1+SMS_MAX_LEN*offset+1, data_buf.len, data_buf.p);
				offset++;
...
void menu_sms_template_display(void)
{
	...
	else if(node.ChildNum <= SCREEN_LINES)
	{
		for(uint8 i=1; i<=node.ChildNum; i++)
		{
			...
			osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(i-1), sizeof(uint8), &data_buf.len);
			osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(i-1)+sizeof(uint8), data_buf.len, data_buf.p);
	...
	else
	{
		for(uint8 i=1; i<=SCREEN_LINES; i++)
		{
			...
			osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(node_info.show_item+i-1), sizeof(uint8), &data_buf.len);
			osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(node_info.show_item+i-1)+sizeof(uint8), data_buf.len, data_buf.p);
...
static void menu_sms_template_edit_display(void)
{
	...
	osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(node_info_temp.sel_item), 1, &data_buf.len);
	osal_nv_read(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*(node_info_temp.sel_item)+1, data_buf.len, data_buf.p);
...
void menu_sms_template_handle_onkey(uint8 keys, uint8 status)
{
	if(keys == HAL_KEY_SELECT)
	{
		...
		{
			osal_nv_write(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*node_info_temp.sel_item, 1, &data_buf.len);
			osal_nv_write(MINEAPP_NV_SMS_TEMPLATE, 1+SMS_MAX_LEN*node_info_temp.sel_item+1, data_buf.len, data_buf.p);
			Buffer_Free(&data_buf);
			menu_JumpBackWithMark();
		}
...
void menu_sms_sendbox_display(void)
{
	...
	else if(node.ChildNum <= SCREEN_LINES)
	{
		...
		for(i=node.ChildNum, j=1; i>0; i--, j++)
		{
			tmp = (i-1)*SMS_MAX_LEN+SMS_LEN_TYPE_SIZE+1;
	...
	else
	{
		...
		for(i=0; i<SCREEN_LINES; i++)
		{
			...
			tmp = (node_info.show_item-i-1)*SMS_MAX_LEN+SMS_LEN_TYPE_SIZE+1;
			osal_nv_read(MINEAPP_NV_SMS_SENDBOX, tmp-1, 1, &data_buf.len);	
			osal_nv_read(MINEAPP_NV_SMS_SENDBOX, tmp, data_buf.len, data_buf.p);
			data_buf.p[data_buf.len] = '\0';
			xpos = LCD_ID_Show(node_info.show_item-i, 0, i+1);
...
static void menu_sms_sendbox_edit_display(void)
{
	...
	osal_nv_read(MINEAPP_NV_SMS_SENDBOX, 1+SMS_MAX_LEN*(node_info_temp.sel_item-1), 1, &data_buf.len);
	osal_nv_read(MINEAPP_NV_SMS_SENDBOX, 1+SMS_MAX_LEN*(node_info_temp.sel_item-1)+1, data_buf.len, data_buf.p);
	data_buf.p[data_buf.len] = '\0';
	menu_shortmessage_writing_display();
}
...
void menu_sms_sendbox_delete_onkey(uint8 keys, uint8 status)
{
	...
	case HAL_KEY_SELECT:
		...
		for(uint8 i=node_info_temp.sel_item; i<offset; i++)
		{
			...
			FeedWatchDog();
#endif
			osal_nv_read(MINEAPP_NV_SMS_SENDBOX, i*SMS_MAX_LEN+1, SMS_MAX_LEN, data);
			osal_nv_write(MINEAPP_NV_SMS_SENDBOX, (i-1)*SMS_MAX_LEN+1, SMS_MAX_LEN, data);
...
#else
		osal_nv_read(MINEAPP_NV_SMS, 0, 1, &node.ChildNum);
#endif
...
static void    menu_shortmessage_read_display(void)
{
	...
	menu_doubleNVID_read(SMS_NVID, len_temp+SMS_LEN_TYPE_SIZE+NMBRDIGIT, data_buf.len, data_buf.p);
	menu_doubleNVID_read(SMS_NVID, len_temp+SMS_LEN_TYPE_SIZE, NMBRDIGIT, num_buf.p);
#else
	osal_nv_read(MINEAPP_NV_SMS, len_temp, 1, &data_buf.len);
	osal_nv_read(MINEAPP_NV_SMS, len_temp+SMS_LEN_TYPE_SIZE+NMBRDIGIT, data_buf.len, data_buf.p);
	osal_nv_read(MINEAPP_NV_SMS, len_temp+SMS_LEN_TYPE_SIZE, NMBRDIGIT, num_buf.p);
#endif
...
#else
	osal_nv_read(MINEAPP_NV_SMS, len_temp-sizeof(uint8), 1, &SMS_flag);
#endif
...
#else
		osal_nv_write(MINEAPP_NV_SMS, len_temp-sizeof(uint8), 1, &SMS_flag);
#endif
...
#else
		osal_nv_read(MINEAPP_NV_SMS, len_temp -sizeof(uint8), sizeof(uint8), &unread_sms);
#endif
		--unread_sms;
#ifdef NEW_DOUBLE_NVID_OP
		menu_doubleNVID_write(SMS_NVID, len_temp-sizeof(uint8), 1, &unread_sms);
#else
		osal_nv_write(MINEAPP_NV_SMS, len_temp-sizeof(uint8), 1, &unread_sms);
#endif
	}
	...
	case HAL_KEY_SELECT:
...
#else
		{
			...
			for(uint8 i=node_info_temp.sel_item; i<offset; i++)
			{
				...
				FeedWatchDog();
#endif
				osal_nv_read(MINEAPP_NV_SMS, i*SMS_NV_LEN+1, SMS_NV_LEN, data);
				osal_nv_write(MINEAPP_NV_SMS, (i-1)*SMS_NV_LEN+1, SMS_NV_LEN, data);

			}
			--offset;
			osal_nv_write(MINEAPP_NV_SMS, 0, 1, &offset);
		}
#endif	
		Buffer_Free(&data_buf);
		NearLastNodeID = CurrentNodeID;
...
void menu_sms_deleteall_onkey(uint8 keys, uint8 status)
{
	...
	case HAL_KEY_SELECT:
		...
		{
			uint8 len = 0;
			osal_nv_write(MINEAPP_NV_SMS, 0, 1, &len);
			uint16 len_tmp = sizeof(uint8)+ MAX_SMS_NUM*SMS_NV_LEN;
			unread_sms = 0;
			osal_nv_write(MINEAPP_NV_SMS, len, sizeof(uint8), &unread_sms);
		}
#endif	
		Buffer_Free(&data_buf);
		NearLastNodeID = CurrentNodeID;
...
#ifdef NEW_DOUBLE_NVID_OP
	//menu_doubleNVID_read(CONTACT_NVID, node_info_temp.sel_item*sizeof(Contact_Node)+1, sizeof(Contact_Node), &c_node);
	menu_Contact_ReadContact(&c_node, node_info_temp.sel_item);
#else
	osal_nv_read(MINEAPP_NV_CONTACT, node_info_temp.sel_item*sizeof(Contact_Node)+1, sizeof(Contact_Node), &c_node);
#endif
...
static void    menu_contact_delete_onkey(uint8 keys, uint8 status)
{
	...
	case HAL_KEY_SELECT:
...
#ifdef NEW_DOUBLE_NVID_OP
		{
			//menu_doubleNVID_delete(CONTACT_NVID, node_info_temp.sel_item);
...
#else     
		{
			uint8 offset, len;
			Contact_Node c_node;
			len = sizeof(Contact_Node);
			osal_nv_read(MINEAPP_NV_CONTACT, 0, 1, &offset);
			--offset;
			osal_nv_write(MINEAPP_NV_CONTACT, 0, 1, &offset);
			for(uint8 i=node_info_temp.sel_item; i<offset; i++)
			{
#if (defined WATCHDOG) &&(WATCHDOG==TRUE)
				FeedWatchDog();
#endif
				osal_nv_read(MINEAPP_NV_CONTACT, (i+1)*len+1, len, &c_node);
				osal_nv_write(MINEAPP_NV_CONTACT, i*len+1, len, &c_node);
			}
		}
#endif
		NearLastNodeID = CurrentNodeID;
...
void menu_bellvolsetting_onkey(uint8 keys, uint8 status)
{
	...
	case HAL_KEY_SELECT:
		...
		HalRingClose();
		//osal_nv_write(MINEAPP_NV_SET_INFORMATION, VOLUME_OFFSET, sizeof(uint8), &volsetting);
		osal_stop_timerEx(Hal_TaskID, MINEAPP_RING_EVENT);
		osal_unset_event(Hal_TaskID, MINEAPP_RING_EVENT);
		strcpy((char *)g_jump_buf,SETTED_CHINA);
		menu_JumpandMark(MENU_ID_SHOWMESSAGE);
...
static void menu_tools_display(void)
{
	...
	else if(node_info.sel_item == 2)  /* show channel */
	{
		...
		{
			_ltoa(_NIB.nwkLogicalChannel , p, 10);
		}
		else
		{
#ifdef MENU_RF_DEBUG
			/*
			uint32 channellist;
			osal_nv_read( ZCD_NV_SET_CHANLIST, 0, sizeof(uint32), &channellist );
			_ltoa(channellist, p, 10);
			*/
	...
	{
		strcpy((char *)p, MINEAPP_SW_VERSION);
		/*
		uint8 len = osal_strlen((char*)MINEAPP_SW_VERSION);
		osal_memcpy(p, MINEAPP_SW_VERSION, len);
		p[len] = '\0';
		*/
	}
#endif
...
number = BUILD_UINT16(aExtendedAddress[0], aExtendedAddress[1]);
_ltoa(number , p, 10);
}
else if(node_info.sel_item == 1) 
{
	...
	case HAL_KEY_SELECT:
		{
			...
			channel = (uint32)((uint32)1<<channel);
			if(ZSUCCESS !=  osal_nv_write( ZCD_NV_SET_CHANLIST, 0, sizeof(uint32), &channel))
			{
				Buffer_Clear(&num_buf);
				menu_steptoparent();
				break;
