 ...
 *
 * COMPONENT:          AppQueueApi.c
 *
 * AUTHOR:             CJG
 *
 * DESCRIPTION:        Application Queue API. Each event type (MLME, MCPS or
 *                     hardware) has two queues defined here, a 'spare buffers'
 *                     queue containing buffers that have not been assigned
 *                     and a 'used buffers' queue holding events waiting to be
 *                     processed by the application.
 *                     When a buffer is needed it is taken from the appropriate
 *                     'spare buffers' queue, filled with information and then
 *                     placed on the 'used buffers' queue.
 *                     When the application asks for the information via one of
 *                     the psAppQApiReadXXXInd functions, the buffer is removed
 *                     from the 'used buffers' queue and passed to the
 *                     application. When the application has finished with the
 *                     information, it calls one of the
 *                     vAppQApiReturnXXXIndBuffer functions to place the buffer
 *                     back on the 'spare buffers' queue.
 *
 ...
 *
 * $Id: AppQueueApi.c,v 1.2 2011/01/11 22:36:29 longshoulun Exp $
 *
 *****************************************************************************
 *
...
#include "system_util.h"		// to register ramdon util
...
PUBLIC void vAppQApiPostHwInt(uint32 u32Device, uint32 u32ItemBitmap)
{
    ...
    {
        prAppHwCallback(u32Device, u32ItemBitmap);
    }

    if((u32Device == E_AHI_DEVICE_SYSCTRL) && (u32ItemBitmap & E_AHI_SYSCTRL_RNDEM_MASK))
 	   vSysCtrlRndCallback();
}
