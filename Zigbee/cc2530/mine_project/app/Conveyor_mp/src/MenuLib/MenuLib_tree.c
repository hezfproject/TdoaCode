...
extern bool  Packge_information;
...
static void menu_conveyor_cmd_demand_display(void);
static void menu_conveyor_cmd_demand_onkey(uint8 keys, uint8 status);
static void menu_conveyor_read_display(void);
static void  menu_conveyor_read_onkey(uint8 keys, uint8 status);
#ifndef ONLY_NEED_STOP      
static void menu_conveyor_sent_display(void);
static void menu_conveyor_sent_onkey(uint8 keys, uint8 status);
static void menu_conveyor_info_display(void);
static void menu_conveyor_info_onkey(uint8 keys, uint8 status);
#endif
...
static char*  const   ItemList_FunctionList[] = {CONTACTLIST_CHINA,CONVEYOR_CHINA,SHORTMESSAGE_CHINA,CALLRECORD_CHINA,SETTINGS_CHINA,TOOLS_CHINA};
#if  1//def MENU_RF_DEBUG
static char*  const   ItemList_Settings[] = {RINGSETTING_CHINA,TIMESETTING_CHINA,BACKLIGHTSETTING_CHINA,PADLOCKSETTING_CHINA,CONTACTUPDATE_CHINA,RESTOREDEFAULT_CHINA,SET_CHANNEL_CHINA, SET_PANID_CHINA};
#else
static char*  const   ItemList_Settings[] = {RINGSETTING_CHINA,TIMESETTING_CHINA,BACKLIGHTSETTING_CHINA,PADLOCKSETTING_CHINA,RESTOREDEFAULT_CHINA};
#endif
...
static char*  const   ItemList_Conveyors[] = {CONVEYOR_CMD_CHINA};//only use CHINA
static char*  const   ItemList_ConveyorHandle[] = {CONVEYOR_CMD_CHINA};
#else
static char*  const   ItemList_Conveyors[] = {CONVEYOR_INFO_CHINA,CONVEYOR_SENT_CHINA,CONVEYOR_CMD_DEMAND_CHINA};
static char*  const   ItemList_ConveyorHandle[] = {CONVEYOR_CMD_CHINA,CONVEYOR_DEMAND_CHINA};
#endif
...
static Tree_node_t  const  Menu_Tree[] =
{
    ...
    ,
    {
        ...
        .ParentID = MENU_ID_FUNCTIONLIST,
#ifdef ONLY_NEED_STOP           
        .FirstChildID =MENU_ID_CONVEYOR_STOP ,
#else
        .FirstChildID =MENU_ID_CONVEYOR_INFO ,
#endif
        .ChildNum = sizeof(ItemList_Conveyors)/sizeof(ItemList_Conveyors[0]),
        .ItemName_CH = ItemList_Conveyors,
        .oper.display = menu_list_display,
 ...
 #ifndef ONLY_NEED_STOP   
    {
        .ID = MENU_ID_CONVEYOR_INFO,
        .name = CONVEYOR_INFO_CHINA,
        .ParentID = MENU_ID_CONVEYOR,
        .FirstChildID = NULL,
        .ChildNum= 0,
        .ItemName_CH = NULL,
        .oper.display = menu_conveyor_info_display,
        .oper.on_key = menu_conveyor_info_onkey,
    }	
    ,
    {
        .ID = MENU_ID_CONVEYOR_SENT,
        .name = CONVEYOR_SENT_CHINA,
        .ParentID = MENU_ID_CONVEYOR,
        .FirstChildID = NULL,
        .ChildNum= 0,
        .ItemName_CH = NULL,
        .oper.display = menu_conveyor_sent_display,
        .oper.on_key = menu_conveyor_sent_onkey,
    }
    ,
        
    {
        .ID = MENU_ID_CONVEYOR_HANDLE,
        .name = CONVEYOR_CMD_DEMAND_CHINA,
        .ParentID = MENU_ID_CONVEYOR,
        .FirstChildID = MENU_ID_CONVEYOR_STOP,
        .ChildNum = sizeof(ItemList_ConveyorHandle)/sizeof(ItemList_ConveyorHandle[0]),
        .ItemName_CH = ItemList_ConveyorHandle,
        //.oper.display = menu_conveyor_handle_display,
       // .oper.on_key = menu_conveyor_handle_onkey,
        .oper.display = menu_list_display,
        .oper.on_key = menu_list_onkey,       
    }
    ,	
    ...
    {
        .ID = MENU_ID_CONVEYOR_STOP,
        .name = CONVEYOR_CMD_CHINA,
 #ifdef ONLY_NEED_STOP           
        .ParentID = MENU_ID_CONVEYOR,
#else
        .ParentID = MENU_ID_CONVEYOR_HANDLE,
#endif
        .FirstChildID = NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
        .oper.display = menu_conveyor_cmd_demand_display,
        .oper.on_key = menu_conveyor_cmd_demand_onkey,
    }
    ,	

 #ifndef ONLY_NEED_STOP       
    {
        .ID = MENU_ID_CONVEYOR_DEMAND,
        .name = CONVEYOR_DEMAND_CHINA,
        .ParentID = MENU_ID_CONVEYOR_HANDLE,
        .FirstChildID = NULL,
        .ChildNum = 0,
        .ItemName_CH = NULL,
    ...
    {
        .ID = MENU_ID_CONVEYOR_READ,
        .name = NULL,
        .ParentID = MENU_ID_CONVEYOR,
        .FirstChildID = NULL,
        .ChildNum= 0,
        .ItemName_CH = NULL,
        .oper.display = menu_conveyor_read_display,
        .oper.on_key = menu_conveyor_read_onkey,
...
static void menu_list_onkey(uint8 keys, uint8 status)
{
    ...
    {
    case HAL_KEY_SELECT:
        if(CurrentNodeID == MENU_ID_CONVEYOR_HANDLE)
            {
                num_buf.p[0] = '\0';
                num_buf.len=0;
            }
    ...
    default:
        if((CurrentNodeID==MENU_ID_SETTINGS)&&!Menu_rf_debug)		   	
	    	menu_list_up_down_onkey(keys, node.ChildNum-2, TRUE);        
	 else	
...
static void    menu_call_record_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_SELECT:
        ...
        else
        {
            ...
            LCD_Str_Print(record.num.Nmbr, 0, 1, TRUE);
...
static void menu_conveyor_sent_display(void)
{
    ...
    menu_Conveyor_Read_Num(&offset,CTYPE_CMD);

    if(NearLastNodeID == MENU_ID_CONVEYOR)  
    {
    ...
    LcdClearDisplay();
    LCD_Str_Print(CONVEYOR_SENT_CHINA, (LCD_LINE_WIDTH-osal_strlen(CONVEYOR_SENT_CHINA))/2, 0, TRUE);
    ...
    else if(node.ChildNum <= SCREEN_LINES)
    {
        ...
        for(i=node.ChildNum, j=1; i>0; i--, j++)
        {
            ...
            menu_Conveyor_Read_Conveyor(&conveyor_s,(i-1),CTYPE_CMD);
            strncpy((char*)num_buf.p,(char *)conveyor_s.date,DATE_LEN);
            //_itoa(conveyor_s.num,(char*)num_buf.p,10);	
#endif
...
#ifdef NEW_DOUBLE_NVID_OP
            Conveyor_sms_t conveyor_s;
            menu_Conveyor_Read_Conveyor(&conveyor_s,node_info.show_item-i-1,CTYPE_CMD);
            strncpy((char*)num_buf.p,(char *)conveyor_s.date,DATE_LEN);
             //_itoa(conveyor_s.num,(char*)num_buf.p,10);	
#endif
...
static void menu_conveyor_sent_onkey(uint8 keys, uint8 status)
{
...
#ifdef NEW_DOUBLE_NVID_OP
    menu_Conveyor_Read_Num(&len,CTYPE_CMD);
#endif
    ...
    case HAL_KEY_SELECT:
    {
        ...
        {
            strcpy((char *)g_jump_buf,"无历史命令信息");
            menu_JumpandMark(MENU_ID_SHOWMESSAGE);
            MP_StartMenuLibEvt(1000);
        }
        break;
...
static void menu_conveyor_cmd_demand_display(void)
{
	LcdClearDisplay();
    	LCD_Clear(8, 0, 16, 3);
	if(MENU_ID_CONVEYOR_STOP==CurrentNodeID)
		{
		    sent_conveyortype=CMDTYPE_STOP;
    		    LCD_Str_Print(INPUT_STOP_CONVEYOR, (LCD_LINE_WIDTH-osal_strlen(INPUT_STOP_CONVEYOR))/2, 1, TRUE);
		}
#ifndef ONLY_NEED_STOP       
		else if(MENU_ID_CONVEYOR_DEMAND==CurrentNodeID)
		{
		    sent_conveyortype=CMDTYPE_DEMAND;
    		    LCD_Str_Print(INPUT_DEMAND_CONVEYOR, (LCD_LINE_WIDTH-osal_strlen(INPUT_DEMAND_CONVEYOR))/2, 1, TRUE);
		}
#endif        
    	LCD_Str_Print(INPUT_CONVEYOR_RANGE, (LCD_LINE_WIDTH-osal_strlen(INPUT_CONVEYOR_RANGE))/2, 3, TRUE);				
    	LCD_Str_Print(INPUT_CONVEYOR_NUMBER, (LCD_LINE_WIDTH-osal_strlen(INPUT_CONVEYOR_NUMBER))/2, 2, TRUE);		
    	LCD_Str_Print(num_buf.p , 0 , 2, FALSE);
    	LCD_ShowCursor(LCD_LINE_WIDTH - 1, 2);
...
static void menu_conveyor_info_display(void)
{
    ...
    Stack_Clear(&global_stack);
     ...
     menu_Conveyor_Read_Num(&offset,CTYPE_INFO);
  ...
  //  if((NearLastNodeID == MENU_ID_SHORTMESSAGE) || (NearLastNodeID == MENU_ID_INCOMINGSMS) ||(NearLastNodeID == MENU_ID_SHORTMESSAGE_DELETE))
    ...
    LcdClearDisplay();
    LCD_Str_Print(CONVEYOR_INFO_CHINA, (LCD_LINE_WIDTH-osal_strlen(CONVEYOR_INFO_CHINA))/2, 0, TRUE);
...
#ifdef NEW_DOUBLE_NVID_OP
            Conveyor_sms_t conveyor_s;
            menu_Conveyor_Read_Conveyor(&conveyor_s,(i-1),CTYPE_INFO);
	     //_itoa(conveyor_s.num,(char*)num_buf.p,10);	
	     strncpy((char*)num_buf.p,(char *)conveyor_s.date,DATE_LEN);
#endif
...
#ifdef NEW_DOUBLE_NVID_OP
            Conveyor_sms_t conveyor_s;
            menu_Conveyor_Read_Conveyor(&conveyor_s,node_info.show_item-i-1,CTYPE_INFO);
            //_itoa(conveyor_s.num,(char*)num_buf.p,10);	
            strncpy((char*)num_buf.p,(char *)conveyor_s.date,DATE_LEN);
#endif
...
#ifdef NEW_DOUBLE_NVID_OP
    menu_Conveyor_Read_Num(&len,CTYPE_INFO);
#endif
...
static void menu_conveyor_cmd_demand_onkey(uint8 keys, uint8 status)
{
    ...
    if(keys == HAL_KEY_SELECT)
    {
        ...
        if(num_buf.len > 0)
        {
            ...
            SET_ON_CONVEYOR_SENDING();
	     Conveyor_sms_t  conveyor_s;
	     GetTimeChar(conveyor_s.time);
             GetDateChar(conveyor_s.date);
	     conveyor_s.num=LO_UINT16(atoul(num_buf.p));	
	     conveyor_s.status=0;
	     conveyor_s.infotype=0;
	     conveyor_s.malfunction_msg=0;
	     conveyor_s.malfunction_type=0;
	     conveyor_s.speed=0;
	     conveyor_s.tensility=0;
         
              if(MENU_ID_CONVEYOR_STOP==CurrentNodeID)
	        {
		    conveyor_s.infotype=INFOTYPE_STOP;
		}         
#ifndef ONLY_NEED_STOP            
	        else if(MENU_ID_CONVEYOR_DEMAND==CurrentNodeID)	 
	     	{
		    conveyor_s.infotype=INFOTYPE_DEMAND;	
		}		
#endif          
	    menu_Conveor_Add(&conveyor_s,CTYPE_CMD);	   
            Stack_Pop(&global_stack, &CurrentNodeID, NULL);
            CurrentNodeID=MENU_ID_MAIN;
	    Stack_Push(&global_stack, CurrentNodeID, NULL);	
...
static void menu_conveyor_read_display(void)
{
    conveyor_type conveyortype=CTYPE_CMD; 	
    uint8 buf[10];
...
#ifndef ONLY_NEED_STOP 
     if(MENU_ID_CONVEYOR_SENT!=NearLastNodeID)
     	{
	conveyortype=CTYPE_INFO;
	if(MENU_ID_CONVEYOR_INFO!=NearLastNodeID)
		{
		uint8 offset;
		menu_Conveyor_Read_Num(&offset,CTYPE_INFO);
		node_info_temp.sel_item=offset;
		node_info_temp.show_item = offset;
        	node_info_temp.high_line= 1;
            	Stack_Push(&global_stack, MENU_ID_MAIN, &node_info);			
		}
	}
#else    
        {
                conveyortype=CTYPE_INFO;
        	if(MENU_ID_CONVEYOR_STOP!=NearLastNodeID)
                {
        		uint8 offset;
        		menu_Conveyor_Read_Num(&offset,CTYPE_INFO);
        		node_info_temp.sel_item=offset;
    ...
    menu_Conveyor_Read_Conveyor(&conveyor_s,node_info_temp.sel_item-1,conveyortype);

     switch(conveyor_s.infotype)
     	{

	case  INFOTYPE_STOP_ACK:
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 3, 0, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:")+3, 0, TRUE);		
		LCD_Str_Print("停止命令操作成功",(LCD_LINE_WIDTH-osal_strlen("停止命令操作成功"))/2, 2,TRUE);		
		break;

	case  INFOTYPE_START:
		LCD_Str_Print("起车预警",(LCD_LINE_WIDTH-osal_strlen("起车预警"))/2, 0,TRUE);	
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 0, 1, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:"), 1, TRUE);		
		LCD_Str_Print("将要启动...",(LCD_LINE_WIDTH-osal_strlen("将要启动..."))/2, 2,TRUE);			
		break;

	case  INFOTYPE_STARTED:
		LCD_Str_Print("起车完成",(LCD_LINE_WIDTH-osal_strlen("起车完成"))/2, 0,TRUE);			
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 0, 1, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:"), 1, TRUE);		
		LCD_Str_Print("启动完成!",(LCD_LINE_WIDTH-osal_strlen("启动完成!"))/2, 2,TRUE);			
		break;

	case  INFOTYPE_MALFUNCTION:
		LCD_Str_Print("故障信息",(LCD_LINE_WIDTH-osal_strlen("故障信息"))/2, 0,TRUE);	
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 0, 1, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:"), 1, TRUE);	
    		//LCD_Str_Print("状态:", 0, 1, TRUE);	
   		// _itoa(conveyor_s.status,buf,10);	
    		//LCD_Str_Print(buf, osal_strlen("状态:"), 1, TRUE);	
    		Get_MalfunctionType_by_number(buf,conveyor_s.malfunction_type);
    		LCD_Str_Print("故障:", 0, 2, TRUE);		
    		LCD_Str_Print(buf, osal_strlen("故障:"), 2, TRUE);	
		if(UPPER_LIMIT==conveyor_s.malfunction_msg)
			{			
			LCD_Str_Print("(上限保护)", (LCD_LINE_WIDTH-osal_strlen("(上限保护)"))/2, 3, TRUE);	
			}
			else if(LOWER_LIMIT==conveyor_s.malfunction_type)	
				{
				LCD_Str_Print("(下限保护)", (LCD_LINE_WIDTH-osal_strlen("(上限保护)"))/2, 3, TRUE);					
				}
		break;

	case  INFOTYPE_STATUS:
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 3, 0, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:")+3, 0, TRUE);	
    		LCD_Str_Print("状态:", 0, 1, TRUE);	
    		//_itoa(conveyor_s.status,buf,10);	
    		if(Get_ConveyorStatus_by_number(buf,conveyor_s.status)==SUCCESS)
    		    {
    		        LCD_Str_Print(buf, osal_strlen("状态:"), 1, TRUE);	
    		    }
                    else
                    {
                        LCD_Str_Print("无", osal_strlen("状态:"), 1, TRUE);	
                    }
    		//Get_MalfunctionType_by_number(buf,conveyor_s.malfunction_type);
    		LCD_Str_Print("速度:", 0, 2, TRUE);	
                uint8 buf1[5],buf2[5];     
                if(conveyor_s.speed)
                    {
		        _itoa(conveyor_s.speed/10,buf1,10);	
                        _itoa(conveyor_s.speed%10,buf2,10);	
		        LCD_Str_Print(buf1, osal_strlen("速度:"), 2, TRUE);	
                        LCD_Str_Print(".", osal_strlen("速度:")+osal_strlen(buf1), 2, TRUE);	
                        LCD_Str_Print(buf2, osal_strlen("速度:")+osal_strlen(buf1)+1, 2, TRUE);	
                        LCD_Str_Print("m/s", osal_strlen("速度:")+osal_strlen(buf1)+osal_strlen(buf2)+1, 2, TRUE);
                    }
                    else
                    {
                        _itoa(conveyor_s.speed,buf,10);	
		        LCD_Str_Print(buf, osal_strlen("速度:"), 2, TRUE);	
                        LCD_Str_Print("m/s", osal_strlen("速度:")+osal_strlen(buf), 2, TRUE);
                    }
		LCD_Str_Print("张力:", 0, 3, TRUE);	
		_itoa(conveyor_s.tensility,buf,10);	
    		LCD_Str_Print(buf, osal_strlen("张力:"), 3, TRUE);		
                LCD_Str_Print("吨", osal_strlen("张力:")+osal_strlen(buf), 3, TRUE);
		break;
        
	case  INFOTYPE_NOCONVEYOR:
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 3, 0, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:")+3, 0, TRUE);		
		LCD_Str_Print("找不到主控基站",(LCD_LINE_WIDTH-osal_strlen("找不到主控基站"))/2, 2,TRUE);		
		break;
        
	case  INFOTYPE_NOREACTION:
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 3, 0, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:")+3, 0, TRUE);		
		LCD_Str_Print("命令无反应",(LCD_LINE_WIDTH-osal_strlen("命令无反应"))/2, 2,TRUE);		
		break;

//sent  display   
	case  INFOTYPE_STOP:
		LCD_Str_Print("急停命令信息", (LCD_LINE_WIDTH-osal_strlen("急停命令信息"))/2, 0, TRUE);
		LCD_Str_Print("时间:", 0, 1, TRUE);	
		LCD_Str_Print(conveyor_s.time, osal_strlen("时间:"), 1, TRUE);	
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 0, 2, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:"), 2, TRUE);	
		LCD_Str_Print("命令:急停", 0, 3, TRUE);				
		break;		
		
	case  INFOTYPE_DEMAND:
		LCD_Str_Print("查询命令信息", (LCD_LINE_WIDTH-osal_strlen("查询命令信息"))/2, 0, TRUE);	
		LCD_Str_Print("时间:", 0, 1, TRUE);	
		LCD_Str_Print(conveyor_s.time, osal_strlen("时间:"), 1, TRUE);		
    		_itoa(conveyor_s.num,buf,10);
    		LCD_Str_Print("皮带号:", 0, 2, TRUE);	
    		LCD_Str_Print(buf, osal_strlen("皮带号:"), 2, TRUE);		
		LCD_Str_Print("命令:查询", 0, 3, TRUE);				
		break;	
