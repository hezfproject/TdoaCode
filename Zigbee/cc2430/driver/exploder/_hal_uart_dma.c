...
#define HAL_UART_DMA_CLR_RX_BYTE(IDX)  (dmaCfg.rxBuf[(IDX)] = BUILD_UINT16(~DMA_PAD, 0))
#else
...
#define HAL_UART_DMA_CLR_RX_BYTE(IDX)  (dmaCfg.rxBuf[(IDX)] = BUILD_UINT16(0, ~DMA_PAD))
#endif
...
static void HalUARTOpenDMA(halUARTCfg_t *config)
{
  ...
  UxBAUD = (config->baudRate == HAL_UART_BR_38400) ? 59 : 216;
  UxGCR = (config->baudRate == HAL_UART_BR_38400) ? 10 : 11;
  ...
  osal_memset(dmaCfg.rxBuf, ~DMA_PAD, HAL_UART_DMA_RX_MAX*2);
...
static uint16 HalUARTWriteDMA(uint8 *buf, uint16 len)
{
  ...
  // If the DMA Tx has finished transferring all scheduled bytes from the other buffer.
