...
typedef struct
{
    ...
    //uint8* txBuf;
    //uint16 txLen;
    uint8_t rxBuf[256];  //add some guard
...
void msg_analyser_register(void (*pfn)( uint8_t* p, uint16_t len))
{
	memset(&UartHdl, 0, sizeof(UartHdl));
	UartHdl.state = UARTSTATE_PREAMBLE_H;
	UartHdl.pbsmac = ( bsmac_header_t *)UartHdl.rxBuf;
	UartHdl.rx_callback = pfn;
}
...
void USART1_IRQHandler(void)
{
	...
	switch(UartHdl.state)
	{
        case UARTSTATE_PREAMBLE_H:
        {
            if(data == BSMAC_PREAMBLE_H)
            {
               UartHdl.pbsmac->preamble_H = data;
                UartHdl.state = UARTSTATE_PREAMBLE_L;
            }
            break;
        }
        case UARTSTATE_PREAMBLE_L:
        {
            ...
            {
                UartHdl.pbsmac->preamble_L  = data;
                UartHdl.state = UARTSTATE_FRAME_CONTROL;
            }
            else
            {
                UartHdl.state = UARTSTATE_PREAMBLE_H;
        ...
        {
            UartHdl.pbsmac->frame_control= data;
            UartHdl.state = UARTSTATE_RESERVERD;
            break;
        }
        case UARTSTATE_RESERVERD:
        {
            UartHdl.pbsmac->reserverd = data;
            UartHdl.state = UARTSTATE_FRAME_COUNT_H;
            break;

        }
        case UARTSTATE_FRAME_COUNT_H:
        {
            UartHdl.pbsmac->frame_count_H = data;
            UartHdl.state = UARTSTATE_FRAME_COUNT_L;
            break;
        }
        case UARTSTATE_FRAME_COUNT_L:
        {
            UartHdl.pbsmac->frame_count_L = data;
            UartHdl.state = UARTSTATE_SRC_ADDR_H;
            break;
        }
        case UARTSTATE_SRC_ADDR_H:
        {
            UartHdl.pbsmac->src_addr_H= data;
            UartHdl.state = UARTSTATE_SRC_ADDR_L;
            break;
        }

        case UARTSTATE_SRC_ADDR_L:
        {
            UartHdl.pbsmac->src_addr_L = data;
            UartHdl.state = UARTSTATE_DST_ADDR_H;
            break;
        }
        case UARTSTATE_DST_ADDR_H:
        {
            UartHdl.pbsmac->dst_addr_H = data;
            UartHdl.state = UARTSTATE_DST_ADDR_L;
            break;
        }

        case UARTSTATE_DST_ADDR_L:
        {
            UartHdl.pbsmac->dst_addr_L = data;
            UartHdl.state = UARTSTATE_DATA_LEN_H;
            break;
        }
        case UARTSTATE_DATA_LEN_H:
        {
            UartHdl.pbsmac->data_len_H = data;
            UartHdl.state = UARTSTATE_DATA_LEN_L;
            break;
        }
        case UARTSTATE_DATA_LEN_L:
        {
            UartHdl.pbsmac->data_len_L = data;

            UartHdl.rx_wantlen = (UartHdl.pbsmac->data_len_H<<8 | UartHdl.pbsmac->data_len_L); // + 2; len is including crc

            if((UartHdl.rx_wantlen <= (UART_MAX_RX_LEN - sizeof(bsmac_header_t))) && (UartHdl.rx_wantlen != 0))
            {
                UartHdl.rx_datalen =0;
                UartHdl.state = UARTSTATE_DATA;
            }
            else
            {
                UartHdl.state = UARTSTATE_PREAMBLE_H;
        ...
        case UARTSTATE_DATA:
        {
            UartHdl.rxBuf[sizeof(bsmac_header_t) + UartHdl.rx_datalen] = data;
            if(++UartHdl.rx_datalen >= UartHdl.rx_wantlen)
            {
                if(UartHdl.rx_callback!=NULL)
                {
				//	PrintfUtil_vPrintf("rx ok\n");
                    UartHdl.rx_callback( UartHdl.rxBuf, sizeof(bsmac_header_t) + UartHdl.rx_datalen);
                }
                UartHdl.state = UARTSTATE_PREAMBLE_H;
            }
            else
