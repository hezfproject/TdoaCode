...
static UINT16 app_CheckIEEEInfo(UINT8 *pu8IEEEAddr)
{
    ...
    ASSERT(pu8IEEEAddr[LPBSS_MAC_CARD_TYPE] == STAFF_CARD_DEVICE_ID);
...
static BOOL app_ReportVerInfo(VOID)
{
    ...
    pstCardVerInfo->u8DevType = STAFF_CARD_DEVICE_ID;
...
static VOID app_HelpSurvey(VOID)
{
    ...
    if (!(s_stAppTxPkt.u8Status & STAFF_STATUS_HELP))
    {
        if (BSP_KEY_IsDown() && app_TestKeyPress(KEY_HELP_TIME))   // 按键按下
        {
            s_u8HelpCnt = HELP_REPORT_MAX;
            s_stAppTxPkt.u8Status |= STAFF_STATUS_HELP;
        }
    }
...
static void app_AdcCheck(void)
{
    ...
    if (++u8HungerCnt == CHECK_ADC_CNT)
    {
        ...
        {
            s_stAppTxPkt.u8Status |= STAFF_STATUS_HUNGER;
        }
        else
        {
            s_stAppTxPkt.u8Status &= ~STAFF_STATUS_HUNGER;
    ...
    {
        event_timer_add(EVENT_ADC_MSG, LOCATE_TIME);
    }
}
...
static void app_LocateProc(void)
{
    ...
    if (!(s_stAppTxPkt.u8Status & STAFF_STATUS_HELP))   // 正常情况下每隔一段时间闪烁一次绿灯
    {
        if (++s_u8FlashLedCnt * REPORT_DELAY >= FLASH_LED_TIME)
        {
            s_u8FlashLedCnt = 0;
            if (!(s_stAppTxPkt.u8Status & STAFF_STATUS_HUNGER))
            {
                BSP_LED_GREEN_Flash();
            }
            else // 低电闪烁红灯
