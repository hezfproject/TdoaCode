...
static void phy_monitor_thread_entry(void *parameter)
{
    ...
    ETH_WritePHYRegister(phy_addr, 0x04, 0x05E1);
    ETH_WritePHYRegister(phy_addr, 0x10, 0x4114);
    ETH_WritePHYRegister(phy_addr, 0x14, 0x0410);

    while(1)
    {
        uint16_t status  = ETH_ReadPHYRegister(phy_addr, PHY_BSR);
        STM32_ETH_PRINTF("PHY status:0x%04X\r\n", status);
        ...
        if(status & (PHY_AutoNego_Complete | PHY_Linked_Status))
        {
            ...
            SR = ETH_ReadPHYRegister(phy_addr, 17);
            STM32_ETH_PRINTF("DM9162 REG 31:0x%04X\r\n", SR);

            SR = (SR >> 12) & 0xF; /* DM9162, REG17[15:12], Speed Indication. */\
...
void rt_hw_stm32_eth_init(void)
{
    ...
    rt_kprintf("ETH MAC ADDR:%02x:%02x:%02x:%02x:%02x:%02x\n",
