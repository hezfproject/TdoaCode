/**************************************************************************************************
  ...
  Revised:        $Date: 2011/08/02 19:49:16 $
...
HAL_ISR_FUNCTION( halUart0RxIsr, URX0_VECTOR )
{
    ...
    //osal_start_timerEx(GasMonitor_TaskID, GASMONITOR_UART_READ0_EVENT, GASMONITOR_FRAME_TIMEOUT);
    //if(!uartWantReadLen)uartWantReadLen=GasMonitor_JudgeUartHeader(&cfg0->rxBuf[cfg0->rxHead] );
    if(!uartWantReadLen)
    {
        switch(cfg0->rxBuf[cfg0->rxHead])
        {
        case UART_PREAMBLE_H:
            Save_cmdtype=0;
            date_flag=0;
            data_checksum=0;
            date_flag|=BV(0);
            cfg0->rxHead++;
            break;
        case UART_PREAMBLE_L:
            if((date_flag&0x01)&&!(date_flag&0x80))
            {
                date_flag|=BV(1);
                date_flag|=BV(7);
                cfg0->rxHead++;
        ...
        default:
            if(((date_flag&0x02)==0x02)&&!(date_flag&0x04))
            {
                date_flag|=BV(2);
                Save_cmdtype=cfg0->rxBuf[cfg0->rxHead];
                cfg0->rxHead++;
            }
            else if(((date_flag&0x04)==0x04)&&!(date_flag&0x08))
            {
                date_flag|=BV(3);
                cfg0->rxHead++;
            }
            else if(((date_flag&0x08)==0x08)&&!(date_flag&0x10))
            {
                date_flag|=BV(4);
                data_len=cfg0->rxBuf[cfg0->rxHead]&0x00ff;
                cfg0->rxHead++;
            }
            else if(((date_flag&0x10)==0x10)&&!(date_flag&0x20))
            {
                date_flag|=BV(5);
                data_len|=cfg0->rxBuf[cfg0->rxHead]&0xff00;
                cfg0->rxHead++;
            }
            else if(((date_flag&0x20)==0x20)&&!(date_flag&0x40))
            {
                date_flag|=BV(6);
                data_checksum=cfg0->rxBuf[cfg0->rxHead]&0x00ff;
                cfg0->rxHead++;
            }
            else if((date_flag&0x40)==0x40)
            {
                date_flag=0;
                data_checksum|=cfg0->rxBuf[cfg0->rxHead]&0xff00;
                if(data_checksum==((uint16)Save_cmdtype+data_len))
                {
                    if(Save_cmdtype==SYNC_REQ)
                    	{
                      osal_start_timerEx(GasMonitor_TaskID, GASMONITOR_UART_SEND_EVENT,10);
			}
                    uartWantReadLen=data_len;
                }
            }
            else
            {
    ...
    if(uartWantReadLen<(128-8)&&uartWantReadLen)
    {
        cfg0->rxHead++;
        --uartWantReadLen;
        if(!uartWantReadLen)
        {
            //receive_uart_buff=cfg0;
            osal_memcpy(receive_uart_buff,cfg0->rxBuf,cfg0->rxHead);
            cfg0->rxHead = 0;
            osal_start_timerEx(GasMonitor_TaskID, GASMONITOR_UART_READ0_EVENT, GASMONITOR_FRAME_TIMEOUT);
        }
