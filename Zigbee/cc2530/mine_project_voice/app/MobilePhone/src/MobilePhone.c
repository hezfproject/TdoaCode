...
#define UARTSTATE_PREAMBLE_Y 0
#define UARTSTATE_PREAMBLE_I 1
#define UARTSTATE_PREAMBLE_R 2
#define UARTSTATE_DATA       3

#define CARD_STATUS_NORMAL   0
#define CARD_STATUS_HELP     1
...
uint8 uartReadBuf[128];
uint8 uartlen;
uint8 uart_recv_state = UARTSTATE_PREAMBLE_Y;
uint8 card_status = CARD_STATUS_NORMAL;
/**************************************************************************************************
*                                           Constant
**************************************************************************************************/
...
static vehicle_rssi_tsD vehicle_rssi_data;
...
void ReportVehicleCardRssi(void);

void SysUtil_vConvertEndian(void* pvData, uint8 u8Len);
uint8 Mp_num_isequal(const app_termNbr_t *p1, const app_termNbr_t *p2);
uint8 BSP_FLASH_Write(uint8 pg, uint16 offset, uint8 *buf, uint16 cnt);
...
void MP_Init(uint8 taskId)
{
    ...
    //vehicle_rssi_data.app_tof_head.msgtype = 42;
    //vehicle_rssi_data.app_tof_head.protocoltype = APP_PROTOCOL_TYPE_CARD;
    vehicle_rssi_data.u16station_addr = MP_DevInfo.nmbr;
    vehicle_rssi_data.u16seqnum = 1;
    ...
    init_uart(HAL_UART_PORT_0,0x08);  //115200


    MP_RFState = MP_RF_OK;
    //MP_start_timerEx(MP_TaskId, MP_UART_READ_EVENT, 50);
    MP_start_timerEx(MP_TaskId,MP_REPORT_VEHICLE_RSSI_EVENT,50);
}
    ...
    unsigned int j;
       ...
       for(i=0; i<APP_NMBRDIGIT; i++)
       {
            for(j=0; j<2; j++)
            {
                ...
                {
                    if((num1 != num2) && ((num1+0x10) != num2)&& ((num1+0x01) != num2))
                    {
                        return 0;
                    }
                }
...
uint16 MP_ProcessEvent(uint8 taskId, uint16 events)
{
    ...
    if(events & MP_UART_READ_EVENT)
    {
        ...
        uartlen = HalUARTRead(HAL_UART_PORT_0,uartReadBuf,127);
        if(uartlen > 0)
        {
            for(i=0; i<uartlen; i++)
            {
                ...
                {
                case UARTSTATE_PREAMBLE_Y:
                    if(data == 'Y')
                        uart_recv_state = UARTSTATE_PREAMBLE_I;
                    break;

                case UARTSTATE_PREAMBLE_I:
                    if(data == 'I')
                        uart_recv_state = UARTSTATE_PREAMBLE_R;
                    else
                        uart_recv_state = UARTSTATE_PREAMBLE_Y;
                    break;

                case UARTSTATE_PREAMBLE_R:
                    if(data == 'R')
                        uart_recv_state = UARTSTATE_DATA;
                    else
                        uart_recv_state = UARTSTATE_PREAMBLE_Y;
                    break;
                ...
                case UARTSTATE_DATA:
                    ...
                    else
                    {
                        if(IS_IDLE() || ON_WAKE())
                        {
                            if(data == 1 && (!AudioIsEnabled()))
                            {
                                //osal_stop_timerEx(MP_TaskId, MP_AUDIO_PROTECT_EVENT);
                                //osal_clear_event(MP_TaskId, MP_AUDIO_PROTECT_EVENT);
                                //MP_start_timerEx(MP_TaskId, MP_AUDIO_PROTECT_EVENT, 8000);
                                
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);//FIXME:To avoid the other side not start normally.
                                card_status = CARD_STATUS_HELP;
                                //HalAudioClose();
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                
                                
                                Hal_StartVoiceBell(VOICEBELL_HELP);
                            }
                            else if(data == 2 && (!AudioIsEnabled()))
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_RETREAT);
                            }
                            else if(data == 4 && (!AudioIsEnabled()))   //超速提醒
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_SPEED);
                            }
                            else if(data == 5 && card_status == CARD_STATUS_HELP)   //求救已发送
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                card_status = CARD_STATUS_NORMAL;
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_HELP_ACK);
                            }
                            else if(data == 8 && (!AudioIsEnabled()))   //UART_CMD_SLOW_DOWM
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_SLOW_DOWM);
                            }
                            else if(data == 9 && (!AudioIsEnabled()))   //UART_CMD_AVOID
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_AVOID);
                            }
                            else if(data == 10 && (!AudioIsEnabled()))   //UART_CMD_GIVE_WAY
                            {
                                MP_ResetAudioStatus(MP_STOP_AUDIO_TIMEOUT);
                                MP_ResetAudio();
                                MP_StopSignalRetrys();
                                Hal_StartVoiceBell(VOICEBELL_GIVE_WAY);
                            }
                        }
                    }
                    uart_recv_state = UARTSTATE_PREAMBLE_Y;
        ...
        //if(keyactive)
        //MP_start_timerEx(MP_TaskId, MP_UART_READ_EVENT, 500);
        return events ^ MP_UART_READ_EVENT;
    }

    /*if(MP_KEYBOARD_STATE_EVENT)
    {
        //extern bool     lastkeyactive;
        //Hal_SetKeyActive(lastkeyactive);
        return events ^ MP_KEYBOARD_STATE_EVENT;
    }*/

    if(MP_REPORT_VEHICLE_RSSI_EVENT)
    {
        ReportVehicleCardRssi();
        MP_start_timerEx(MP_TaskId, MP_REPORT_VEHICLE_RSSI_EVENT,1000);
        return events ^ MP_REPORT_VEHICLE_RSSI_EVENT;
    }
    return 0;
...
void MP_DeviceStartup()
{
    ...
    /* Setup shortAddr */
    uint16 MobilePanid =  MP_DevInfo.nmbr;
    //MAC_MlmeSetReq(MAC_SHORT_ADDRESS, &ShortAddr);
    MAC_MlmeSetReq(MAC_PAN_ID, &MobilePanid);

    /* Setup PAN ID */
    uint16 MobileShortAddr = 0x0000;
    //MAC_MlmeSetReq(MAC_PAN_ID, &MobilePanID);
    MAC_MlmeSetReq(MAC_SHORT_ADDRESS, &MobileShortAddr);
...
byte MP_ParseAppFrame(const sData_t data, uint16 panid, uint16 shortAddr, int8 rssi)
{
    ...
    {
        //if(pHeader->protocoltype == APP_PROTOCOL_TYPE_CARD && ((IS_IDLE() || ON_WAKE())))
        if(pHeader->protocoltype == APP_PROTOCOL_TYPE_CARD)
        {
            if(pHeader->msgtype == 0x05) // rssi
            {
                RfTofWrapper_tu* psAppPkt = (RfTofWrapper_tu*)(data.p);

                vehicle_rssi_data.vehicle_rssi[u16RssiNum].u16ShortAddr = shortAddr;
                vehicle_rssi_data.vehicle_rssi[u16RssiNum].i8Rssi = rssi;
                vehicle_rssi_data.vehicle_rssi[u16RssiNum].u8Reserved = 0;
                vehicle_rssi_data.vehicle_rssi[u16RssiNum].uu16SeqNum = psAppPkt->rf_tof_card_data.u16SeqNum;
                SysUtil_vConvertEndian(&vehicle_rssi_data.vehicle_rssi[u16RssiNum].uu16SeqNum,sizeof(uint16));
                ...
                {
                    ReportVehicleCardRssi();
                    /*vehicle_rssi_data.u16seqnum++;
                    MP_SendTunnelToAllCoord((uint8 *)(&vehicle_rssi_data),u16RssiNum*sizeof(vehicle_rssi_ts)+4,42,true);
                    u16RssiNum = 0;*/
                }
                return ZSUCCESS;
            }
    ...
    {
        mp_Scan_t *p = (mp_Scan_t *)pPayload;
        if(MP_DevInfo.Channel != p->scantype)
		{
            if(p->scantype >= 11 && p->scantype < 26)
            {
               MP_DevInfo.Channel = p->scantype;
               HalFlashErase((HAL_NV_PAGE_END - HAL_NV_PAGE_CNT + 1));
               BSP_FLASH_Write((HAL_NV_PAGE_END - HAL_NV_PAGE_CNT + 1), 0,(uint8 *)&MP_DevInfo.Channel, 1);
               MAC_MlmeSetReq(MAC_LOGICAL_CHANNEL, &MP_DevInfo.Channel);
            }
...
//#endif
    ...
    case MP_DOWN_ACCEPT:
    {
        if(Mp_num_isequal(&MP_AudioInfo.peer_termnbr, &pCmd->srcnbr))  /* command from peer */
        {
    ...
    case MP_DOWN_CALL:
    {
        ...
        {
            if(Mp_num_isequal(&pCmd->srcnbr, &MP_AudioInfo.peer_termnbr)) //audio started, it is peer downcall retrans
            {
...
static  uint8  MP_ReadExtAddr(void)
{
    ...
    HalFlashRead((HAL_NV_PAGE_END - HAL_NV_PAGE_CNT + 1), 0,(uint8 *)&u8Setchannel, 1);

    if((u8Setchannel>=11 && u8Setchannel <=26))
    {
        channel =  u8Setchannel;
    }
...
void MP_ResetAudioStatus(uint16 timeout)
{
    ...
    osal_clear_event(MP_TaskId, MP_STOP_AUDIO_EVENT);
