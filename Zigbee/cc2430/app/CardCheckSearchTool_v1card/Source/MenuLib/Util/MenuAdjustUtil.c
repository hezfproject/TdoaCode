...
#if CFG_GAS_DETECTOR || CFG_GAS_FIND_APPOINT_CARD
...
void    menu_calibration_setdensity_display(void)
{
    ...
    */
    ...
    if(FIRSTTIME_INTO_NODE())
    {
        ...
        LCD_Str_Print_Pixel((uint8 *)buf, GASDISP_START_X+10, (uint8)(1.8*LCD_LINE_HIGH));
        ch4_getnum_fromdensity(&n1,&n2,&n3, adjustgas_density);
        menu_GasPrint(n1,n2,n3,true);
    }
    ...
    if(adjustgas_invpos ==0)
    {
        x = GASDISP_START_X;
        y = GASDISP_START_Y;
        LCD_Area_Inv(x,x+1, (uint8)(1.2*y*LCD_LINE_HIGH),(uint8)((y+1.8)*LCD_LINE_HIGH));
    }
    else if(adjustgas_invpos ==1)
    {
        x = GASDISP_START_X+3;
...
void    menu_settings_overdensity_display(void)
{
    ...
    LCD_Area_Inv(GASDISP_START_X,GASDISP_START_X+6, (uint8)(1.2*GASDISP_START_Y*LCD_LINE_HIGH),(uint8)((GASDISP_START_Y+1.8)*LCD_LINE_HIGH));
}
...
void    menu_settings_temperature_display(void)
{
    ...
    {
        LCD_Area_Inv(TEMPERDISP_START_X+4,TEMPERDISP_START_X+8, (uint8)(1.2*TEMPERDISP_START_Y*LCD_LINE_HIGH),(uint8)((TEMPERDISP_START_Y+1.8)*LCD_LINE_HIGH));
    }
    else if(adjusttemper>-100 &&adjusttemper<0 || adjusttemper>=100)
    {
        LCD_Area_Inv(TEMPERDISP_START_X+2,TEMPERDISP_START_X+8, (uint8)(1.2*TEMPERDISP_START_Y*LCD_LINE_HIGH),(uint8)((TEMPERDISP_START_Y+1.8)*LCD_LINE_HIGH));
    }
    else if(adjusttemper<=-100)
    {
        LCD_Area_Inv(TEMPERDISP_START_X,TEMPERDISP_START_X+8, (uint8)(1.2*TEMPERDISP_START_Y*LCD_LINE_HIGH),(uint8)((TEMPERDISP_START_Y+1.8)*LCD_LINE_HIGH));
    }
}
...
static char cardNum_t[] = {'0','0','0','0','0','\0'};
uint16 cardNum_u16 = 0;
static uint8 adjustcardnum_invpos = 0;

/*页面显示:按键调整要搜索卡的卡号*/
void menu_CardNumAdjust_display(void)
{
    /*卡号 万千百十一位 -> pos_0/1/2/3/4*/
    const uint8 X_START = 5;
    const uint8 Y_START = 2;

    const uint8  pos_0 = X_START;
    const uint8  pos_1 = pos_0 + 1;
    const uint8  pos_2 = pos_1 + 1;
    const uint8  pos_3 = pos_2 + 1;
    const uint8  pos_4 = pos_3 + 1;

    if(FIRSTTIME_INTO_NODE())
    {
        adjustcardnum_invpos = 0;
        LcdClearDisplay();
    }

    if(!is_achieve_cardnumber)
    {
        LCD_Str_Print((uint8 *)CARDNUMSEARCH_CHINA, (LCD_LINE_WIDTH-osal_strlen((char *)CARDNUMSEARCH_CHINA))/2, 0,TRUE);
        LCD_Str_Print((uint8 *)INPUT_CARD_NUMBER_CHINA, 0, 1,TRUE);
        LCD_Str_Print((uint8 *)":", osal_strlen((char *)INPUT_CARD_NUMBER_CHINA), 1,TRUE);
        LCD_Str_Print((uint8 *)cardNum_t, X_START, Y_START, TRUE);
        LCD_Str_Print((uint8 *)"范围 1-65000", 0, 3, TRUE);

        if(adjustcardnum_invpos ==0)  // 万位
        {
            LCD_Char_Inv(pos_0, Y_START,1);
        }
        else if(adjustcardnum_invpos ==1)
        {
            LCD_Char_Inv(pos_1, Y_START,1);
        }
        else if(adjustcardnum_invpos ==2)
        {
            LCD_Char_Inv(pos_2, Y_START,1);
        }
        else if(adjustcardnum_invpos ==3)
        {
            LCD_Char_Inv(pos_3, Y_START,1);
        }
        else if(adjustcardnum_invpos ==4)
        {
            LCD_Char_Inv(pos_4, Y_START,1);
        }
    }
    else
    {
        if(cardNum_u16 == 0 || cardNum_u16 > 65000)
        {
            is_achieve_cardnumber = false;
            LCD_Line_Clear(3);
            LCD_Str_Print((uint8 *)"卡号错误", 0, 3, TRUE);
            LCD_Char_Inv(pos_4, Y_START,1);
        }
        else
        {
...
{
    uint8 max;
    uint8 pos = adjustcardnum_invpos;
    ...
    {
        if(cardNum_t[0]=='6')max=5;
        else max=9;
    }
    else if(pos == 2)
    {
        if((cardNum_t[0]=='6')&&(cardNum_t[1]>'4'))
            max=5;
        else max=9;
    }
    else if(pos == 3)
    ...
    {
        cardNum_t[pos]++;
        if(cardNum_t[pos]>max+'0') cardNum_t[pos] = '0';

        cardNum_u16 = (cardNum_t[0]-'0')*10000+(cardNum_t[1]-'0')*1000+
            (cardNum_t[2]-'0')*100+(cardNum_t[3]-'0')*10+(cardNum_t[4]-'0');
    ...
    case HAL_KEY_DOWN:
        cardNum_t[pos]--;
        if(cardNum_t[pos]<'0') cardNum_t[pos] = max+'0';
        ...
        break;
    case HAL_KEY_OK:
        if(adjustcardnum_invpos < 4)
        {
            adjustcardnum_invpos++;
            NearLastNodeID = CurrentNodeID;
            menu_display();
        }
        else
        ...
        break;
    case HAL_KEY_MODE:
        if(adjustcardnum_invpos == 0)
        {
            adjustcardnum_invpos=4;
        }
        else
        {
            adjustcardnum_invpos--;
