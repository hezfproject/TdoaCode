...
PRIVATE void Rx_State (uint8 u8Rx_data)
{
	...
	case RX_ADDRESS_L_STATE:
		...
		if (u16Big_To_Little(u16rx_slv_id) == MbusSlvSensorAddr)
		{
...
PUBLIC void Send_Pack (uint8 fram_control)
{
    ...
    mbus_hdr_slv.slv_id = u16Big_To_Little(MbusSlvSensorAddr);
    ...
    if (mbus_hdr_mstr_p->cmd == MBUS_CMD_CLR)
    {
        ...
        //PrintfUtil_vPrintf("Self_crc_send_clr  %x \r\n",crc_send_clr );
    }
    if (mbus_hdr_mstr_p->cmd == MBUS_CMD_QRY)
    {
        ...
        //PrintfUtil_vPrintf("Self_crc_send_qry  %x \r\n",crc_send_qry );
    }
    //PrintfUtil_vPrintf("mbus_hdr_slv_p->data_len  %x \r\n",mbus_hdr_slv_p->data_len );
}
