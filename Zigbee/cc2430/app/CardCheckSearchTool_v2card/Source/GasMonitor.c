...
extern uint16       gas_cardcheck_num;
#endif
...
uint16 GasMonitor_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    {
        //离上次获取信号的rssi超过6秒，超时复位。
        updataSearchCardOvertime();
        menu_JumptoItself();
...
#ifdef CFG_GAS_CARDCHECK
    if(events & GASMONITOR_CHECKCARD_TIMER_EVENT)
    {
        gas_cardcheck_num = 0xffff;
        return events ^ GASMONITOR_CHECKCARD_TIMER_EVENT;
    }
#endif
...
void GasMonitor_ProcessMSGCB(const afIncomingMSGPacket_t *MSGpkt)
{
    ...
    case  CHARGEED_SSREQ:
    {
        ...
        if(app_chargeed_ssReq->srcPan==CARD_NWK_ADDR)
        {
            ...
            //if( MENU_ID_V1_CARDSEARCH_RESULT==CurrentNodeID
            //    || MENU_ID_V1_CARDNUMSEARCH_RESULT==CurrentNodeID
            //    || MENU_ID_V1CARDCHECK==CurrentNodeID)
            //{
...
void GasMonitor_Chece_V2Card(const afIncomingMSGPacket_t *MSGpkt)
{
    ...
    if((Save_CardNum!=MSGpkt->groupId)||diff_time>300)
    {
        if( MENU_ID_V2_CARDSEARCH_RESULT==CurrentNodeID
            ...
            || MENU_ID_CARDCHECK==CurrentNodeID)
        {
...
{
    uint8 len = sizeof(RSSI_table)/sizeof(RSSI_table[0]);   //4
    uint8 i;
...
/*
*   根据RSSI转换成距离，最大255，超过的都用255表示。
*/
uint16 GasMonitor_RSSI2Distance(int8 RSSI)
{
    return (uint16)pow(10, (double)RSSI/(-45));
}
