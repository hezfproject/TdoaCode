...
Revised:        $Date: 2011/07/31 04:55:01 $
...
static uint8* MineApp_SPIReadBuf;
static uint8* MineApp_RfBuf;
...
void MineApp_Init ( uint8 task_id )
{
    ...
    spiConfig.taskID = MineApp_TaskID;
    spiConfig.eventID=0; 
    ...
    /*alloc rf buffer */
    MineApp_SPIReadBuf = osal_mem_alloc(DMA_RT_MAX+10);
    MineApp_RfBuf = osal_mem_alloc(MAX_DATA_SIZE+10);
...
#if 0	
    if(events & MINEAPP_SPI_READ_EVENT)
    {
        uint16 len;
        if(HalSPIRead((uint8*)MineApp_SPIReadBuf, &len) == ZSUCCESS)
        {
            MineApp_ProcessSPICB ( ( SPIMSGPacket_t * ) MineApp_SPIReadBuf );
        }
        return  events ^ MINEAPP_SPI_READ_EVENT;
    }
#endif	
    ...
    case CHARGEED_SSREQ:
    {
        ...
        if ( reqtype == SSREQ_POLL )
        {
            ...
            MAC_UTIL_BuildandSendDataPAN ( &param, ( uint8 * ) &app_chargeed_ssRsp, sizeof(app_chargeed_ssRsp_t),
                                           MAC_UTIL_BROADCAST, MAC_UTIL_BROADCAST_SHORTADDR_DEVALL, MAC_TXOPTION_NO_CNF );
        ...
        if ( reqtype & SSREQ_POLL )
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    MAC_UTIL_BuildandSendDataPAN ( &param, ( uint8 * ) &app_ssReq, sizeof(app_ssReq),
                                                   MAC_UTIL_BROADCAST, MAC_UTIL_BROADCAST_SHORTADDR_DEVALL, MAC_TXOPTION_NO_CNF );
                }
            }
        }
