...
static uint8 Alarmtype;
static uint8 firstCome=0;
...
static void    menu_locate_onkey(uint8 keys, uint8 status);
#endif
...
#endif
#ifdef CFG_GAS_INFORMATION
static char*  const __code  ItemList_tools[] = {GAS_NUMBER_CHINA, GAS_CHANNEL_CHINA, GAS_CPU_VERSION_CHINA,GAS_RADIO_VERSION_CHINA,GAS_RSSI_CHINA};
#endif
...
static Tree_node_t  const __code Menu_Tree[] =
{
    ...
    */
#ifdef  TURN_ON_LOCATE_DISPLAY
    {
        ...
        .oper.on_key = menu_locate_onkey, //menu_functionlist_onkey,
    }
    ,
#endif
...
static void menu_tools_display(void)
{
    ...
    else if(node_info.sel_item == 3)  /* show software version */
    {
        strcpy((char *)p, radioVersion);
    }    
    else if(node_info.sel_item == 4)/* show Network RSSI */
    {        
        uint8 *pData=p;
        
        if(GasMonitor_Get_Network_Rssi() == NO_RSSI)
            {
                strcpy((char *)p, "Ã»ÓÐÍøÂç");
            }
            else
            {
                    if(GasMonitor_Get_Network_Rssi()<0)
                    {
                         *pData++='-';
                    }
                    menu_itoa((uint16)(-GasMonitor_Get_Network_Rssi()) , (char*)pData);
            }
    }
    menu_selectlist_display(p);
}
...
static void menu_SOSAlarm_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_OK:
        ...
        {
        case 0:
            Alarmtype=STATUS_FIRE;
            break;

        case 1:
            Alarmtype=STATUS_WATER;
            break;

        case 2:
            Alarmtype=STATUS_TOPBOARD;
            break;

        case 3:
            Alarmtype=STATUS_OTHER;
            break;
...
void menu_locate_display()
{
    ...
    LCD_Str_Print((uint8 *)LOCATEINFORMATION_CHINA, (LCD_LINE_WIDTH-osal_strlen((char *)LOCATEINFORMATION_CHINA))/2, 0,TRUE);
    if((GasMonitor_Get_SAVE_LocateDistance()<1000)&&locateIsValid)
    {
        ...
        if(firstCome)
        {
            LCD_ListLine_Clear(2);
            LCD_ListLine_Clear(3);
        }
    ...
    {
        LcdClearDisplay();
        LCD_Str_Print((uint8 *)LOCATEFAILURE_CHINA, (LCD_LINE_WIDTH-osal_strlen((char *)LOCATEFAILURE_CHINA))/2, 2,TRUE);
    }
}
...
{
    firstCome=0;
    menu_functionlist_onkey(keys,status);
}
#endif
