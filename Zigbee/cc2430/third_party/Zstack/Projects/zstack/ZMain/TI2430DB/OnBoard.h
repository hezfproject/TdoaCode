/**************************************************************************************************
  Filename:       OnBoard.h
  Revised:        $Date: 2010/10/06 22:42:11 $
...
#ifdef CHARGEED_PROJECT
#define OSAL_SET_CPU_INTO_SLEEP(timeout) st(if(timeout>0 && timeout<2*CHARGEED_BLAST_PERIOD) UtilSleepWithMac(CC2430_PM1,timeout);)
#else
#define OSAL_SET_CPU_INTO_SLEEP(timeout) Util_osal_sleep(timeout);//halSleep(timeout); /* Called from OSAL_PwrMgr */
#endif
...
/* External I/O Processing Functions */
  ...
  */
  extern __near_func void Onboard_TimerCallBack ( uint8 timerId, uint8 channel, uint8 channelMode);
