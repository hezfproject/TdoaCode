...
void MLN_Main_TimerCallBack(uint8 timerId, uint8 channel, uint8 channelMode);

/* This callback is triggered when a key is pressed */
void MLN_Main_KeyCallback(uint16 keys, uint8 state);
...
int main(void)
{
  ...
  HalTimerConfig ( OSAL_TIMER,                         // 8bit timer3
                   ...
                   FALSE,                              // Use interrupt
                   MLN_Main_TimerCallBack);            // Channel Mode

  /* Setup Keyboard callback */
  HalKeyConfig(MLN_KEY_INT_ENABLED, MLN_Main_KeyCallback);
...
void MLN_Main_KeyCallback(uint16 keys, uint8 state)
{
  if ( MLN_TaskId != TASK_NO_TASK )
  {
    MLN_HandleKeys (keys, state);
  }
}
 ...
 *
 **************************************************************************************************/
void MLN_PowerMgr(uint8 enable)
{
