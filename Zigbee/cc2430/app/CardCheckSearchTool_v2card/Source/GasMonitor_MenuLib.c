...
void Menu_handle_msg(uint8 MSG, uint8 *p, uint8 len)
{
    ...
    case MSG_INIT_MAIN:
    {
        ...
        if(CurrentNodeID==MENU_ID_CARDSEARCH
                ||CurrentNodeID==MENU_ID_V2_CARDSEARCH_RESULT)
        {
            menu_CardSearch_end();
        }
...
#endif
    else if(NearLastNodeID == MENU_ID_V2_CARDSEARCH_RESULT
        ||NearLastNodeID == MENU_ID_V2_CARDNUMSEARCH_RESULT
#ifdef CFG_GAS_SOSALARM
...
#endif
    else if(NearLastNodeID == MENU_ID_V2CARDSEARCH)
    {
        menu_JumptoMenu(MENU_ID_V2_CARDSEARCH_RESULT);
    }
    else if(NearLastNodeID == MENU_ID_V2CARDNUMSEARCH)
    {
        menu_JumptoMenu(MENU_ID_V2_CARDNUMSEARCH_RESULT);
    }
