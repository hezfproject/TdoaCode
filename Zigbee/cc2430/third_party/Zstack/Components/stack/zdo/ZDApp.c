/**************************************************************************************************
  Filename:       ZDApp.c
  Revised:        $Date: 2011/01/12 00:43:33 $
...
ZStatus_t ZDO_NetworkDiscoveryConfirmCB2( byte ResultCount,networkDesc_t *NetworkList );
/*********************************************************************
 * LOCAL VARIABLES
 */
...
void ZDApp_ProcessOSALMsg( osal_event_hdr_t *msgPtr )
{
  ...
  switch ( msgPtr->event )
  {
    ...
    case ZDO_NWK_JOIN_REQ:
      ...
      //when receive ZDO_SyncIndictionCB,
      //start a new associate nwk-join instead of an orphan nwk-join.
...
#if defined ( ZDO_COORDINATOR ) && defined ( REFLECTOR )
    case Bind_rsp:
    case Unbind_rsp:
      if ( matchED )
      {
 ...
 */
 #if ((defined HAL_AUDIO) && (HAL_AUDIO == TRUE)) || (defined MINE_CONTROLLER)
 void ZDO_SetOnceScan(bool val)
{
	Zdo_OnceScan = val;
}
 bool ZDO_IsOnceScan(void)
{
	return Zdo_OnceScan;
}
#endif
...
ZStatus_t ZDO_NetworkDiscoveryConfirmCB( byte ResultCount, networkDesc_t *NetworkList )
{
    ...
    if(Zdo_OnceScan)
    {
           MineApp_NetworkDiscoveryConfirmCB(ResultCount, NetworkList);
           return ZSuccess;
    }
    else
    {
            return ZDO_NetworkDiscoveryConfirmCB2(ResultCount, NetworkList);
    }
#else
    return ZDO_NetworkDiscoveryConfirmCB2(ResultCount, NetworkList);
#endif
...
ZStatus_t ZDO_NetworkDiscoveryConfirmCB2( byte ResultCount,
...
#if 1
	//rewrite startupfromapp logic.
	if (mode == APP_STARTUP_COORD)
	{
		devStartMode = MODE_HARD;
	}
	else
