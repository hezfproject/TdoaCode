...
void    menu_inputchinese_display(void)
{
    ...
    {
        if(last_input_status==CHINESE_IN)LCD_Str_Print(PINYIN_CHINA, 0, 0, TRUE);
        if(last_input_status==LETTER_IN)LCD_Str_Print(ENGLISH_CHINA, 0, 0, TRUE);
        if(last_input_status==PUNCTUATION_IN)LCD_Str_Print(PUNCTUATION_CHINA, 0, 0, TRUE);
        if(last_input_status==NUMBER_IN)LCD_Str_Print(NUMBER_CHINA, 0, 0, TRUE);
    }
    letter_id = 0;
        ...
        if(input_status == CHINESE_BACK)
        {
            ...
            else
            {
                if((result =  py_ime(inputstr)) != 0)
                {
                    ...
                    memcpy((void *)chinese_output, (void *)(result + ((chinese_select / CHINESE_BACK_LEN) - 1)*CHINESE_BACK_LEN), CHINESE_BACK_LEN);
        ...
        else if(input_status == CHINESE_BACK)
        {
            if(((chinese_select / CHINESE_BACK_LEN + 1)*CHINESE_BACK_LEN) < chinese_len)
            {
                if((result =  py_ime(inputstr)) != 0)
                {
                    ...
                    memcpy((void*)chinese_output, (void*)(result + ((chinese_select / CHINESE_BACK_LEN) + 1)*CHINESE_BACK_LEN), len);
    ...
    case HAL_KEY_BACKSPACE:
        if((input_status == CHINESE_IN) || (input_status == CHINESE_BACK))
        {
            ...
            else if(letter_id > 1)
            {
                ...
                {
                    LCD_BigAscii_Print(0x11, 0, 3);
                    memcpy((void*)chinese_output, (void*)result, 14);
...
static void menu_ChineseProcessKeyLeft(void)
{
    if(input_status == CHINESE_BACK)
    {
        ...
        else if(chinese_select == 0)
        {
            ...
            {
                LCD_Line_Clear(3);
                memcpy((void*)chinese_output, (void*)(result + (chinese_len / CHINESE_BACK_LEN)*CHINESE_BACK_LEN), chinese_len % CHINESE_BACK_LEN);
        ...
        if(chinese_select < chinese_len - 2)
        {
            ...
            if((chinese_select % 14) == 0)
            {
                if((result =  py_ime(inputstr)) != 0)
                {
                    ...
                    LCD_Line_Clear(3);
                    memcpy((void*)chinese_output, (void*)(result + (chinese_select / 14) * 14), len);
                    LCD_Memory_Print(chinese_output, len, 1, 3);
                    LCD_BigAscii_Print(0x11, 0, 3);
                    LCD_BigAscii_Print(0x10, 15, 3);
                }
