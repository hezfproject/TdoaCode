/**************************************************************************************************
  Filename:       OSAL_Nv.c
  Revised:        $Date: 2009/02/07 01:08:32 $
...
/**************************************************************************************************
  Notes:
    ...
    - Due to .s51 code and code-size saving tricks, the max NV item size is 1024 bytes.
**************************************************************************************************/
...
/* This can be placed in the non-retention RAM - the assembly routine is copied from Flash to XDATA
 * on every call. If enough retention RAM is available, or if on a mains-powered device, could
 * easily change to copy Flash to XDATA only on initialization.
 */
uint8 __xdata flashXdataBuf[22];
 ...
 * @fn      nvRead...
 *
 * @brief   Reads a byte, OSAL_NV_WORD_SIZE, or OSAL_NV_HDR_SIZE bytes from NV.
 *
 * @param   pg - Valid NV page.
 * @param   offset - Valid offset into the page.
 * @param   buf - Valid buffer space at least as big as the read request.
 *
 * @return  none
 */
extern __near_func void nvReadByte( uint8 pg, uint16 offset, uint8 *buf );
extern __near_func void nvReadWord( uint8 pg, uint16 offset, uint8 *buf );
extern __near_func void nvReadHdr(uint8 pg, uint16 offset, uint8 *buf);

/*********************************************************************
 * @fn      nvReadBuf
 *
 * @brief   Reads N bytes from NV.
 *
 * @param   pg - Valid NV page.
 * @param   offset - Valid offset into the page.
 * @param   buf - Valid buffer space at least as big as the read request.
 ...
 * @fn      flashWriteBuf
 *
 * @brief   Writes N blocks of 4-bytes to NV.
 *
 * @param   cnt - Number of blocks of 4-bytes to write.
 * @param   buf - Valid buffer space at least as big as the write request.
 * @param   addr - Valid FADDRH/L address: 3-byte addr / 4 = 2-byte addr.
 *
 * @return  none
 */
extern __near_func void flashWriteBuf(uint8 cnt, uint16 buf, uint16 addr);
...
static void   writeWordM( uint8 pg, uint16 offset, uint8 *buf, uint8 cnt );
...
static uint8  writeIEEE( void *buf );
...
static uint8 initNV( void )
{
  ...
  nvReadHdr( OSAL_NV_IEEE_PAGE, OSAL_NV_IEEE_OFFSET, (uint8 *)(&ieee) );
  ...
  {
    nvReadHdr( pg, OSAL_NV_PAGE_HDR_OFFSET, (uint8 *)(&pgHdr) );
    ...
    nvReadHdr( pg, OSAL_NV_IEEE_OFFSET, (uint8 *)(&pgHdr) );
    if ( xBad )
    {
      ...
      {
        writeWordM( OSAL_NV_IEEE_PAGE, OSAL_NV_IEEE_OFFSET, (uint8 *)(&pgHdr), 2 );
        xBad = FALSE;
      }
    }
    else
    {
      writeWordM( pg, OSAL_NV_IEEE_OFFSET, (uint8 *)(&ieee), 2 );
    }
  }  // for ( pg = OSAL_NV_PAGE_BEG; pg <= OSAL_NV_PAGE_END; pg++ )
...
static void compactPage( uint8 srcPg )
{
  ...
  {
    uint16 sz;
    nvReadHdr( srcPg, srcOff, (uint8 *)(&hdr) );
...
static void writeWord( uint8 pg, uint16 offset, uint8 *buf )
{
    ...
    if ( OSAL_NV_CHECK_BUS_VOLTAGE )
    {
    flashWriteBuf(1, (uint16)buf, offset);
    }
    else
    {
      failF = TRUE;
...
static void writeWordM( uint8 pg, uint16 offset, uint8 *buf, uint8 cnt )
{
  ...
  if ( OSAL_NV_CHECK_BUS_VOLTAGE )
  {
    flashWriteBuf(cnt, (uint16)buf, offset);
  }
  else
  {
    failF = TRUE;
...
static void writeBuf( uint8 dstPg, uint16 dstOff, uint16 len, uint8 *buf )
{
  ...
  if ( len )
  {
    writeWordM( dstPg, dstOff, buf, (uint8)len );
...
static void xferBuf( uint8 srcPg, uint16 srcOff, uint8 dstPg, uint16 dstOff, uint16 len )
{
  ...
  if ( rem )
  {
    ...
    while ( (rem < OSAL_NV_WORD_SIZE) && len )
    {
      nvReadByte( srcPg, srcOff, tmp+rem );
      srcOff++;
      rem++;
      len--;
    }
  ...
  if ( rem )
  {
    ...
    while ( rem-- )
    {
      nvReadByte( srcPg, srcOff, tmp+idx );
      srcOff++;
      idx++;
    }
    writeWord( dstPg, dstOff, tmp );
  ...
  if ( (hdr.id == id) && (hdr.len == len) )
  {
    if ( flag )
    {
      ...
      if ( chk == calcChkF( pg, offset, len ) )
      {
        ...
        nvReadHdr( pg, (offset-OSAL_NV_HDR_SIZE), (uint8 *)(&hdr) );
 ...
 * @fn      writeIEEE
 *
 * @brief   Writes special location / protocol for the IEEE. The global 'failF' should have been
...
{
  osalNvHdr_t hdr;
  nvReadHdr( OSAL_NV_IEEE_PAGE, OSAL_NV_IEEE_OFFSET, (uint8 *)(&hdr) );

  if ( (hdr.id == OSAL_NV_ERASED_ID) && (hdr.len == OSAL_NV_ERASED_ID) &&
       (hdr.chk == OSAL_NV_ERASED_ID) && (hdr.stat == OSAL_NV_ERASED_ID) )
  {
    writeWordM( OSAL_NV_IEEE_PAGE, OSAL_NV_IEEE_OFFSET, buf, 2 );
...
uint8 osal_nv_item_init( uint16 id, uint16 len, void *buf )
{
  ...
  // ZCD_NV_EXTADDR is maintained without an osalNvHdr_t, so it is always already initialized.
...
uint16 osal_nv_item_len( uint16 id )
{
  ...
  else
  {
    ...
    {
      osalNvHdr_t hdr;
      nvReadHdr( findPg, (offset - OSAL_NV_HDR_SIZE), (uint8 *)(&hdr) );
      return hdr.len;
    }
  }
}
...
uint8 osal_nv_write( uint16 id, uint16 ndx, uint16 len, void *buf )
{
  ...
  if ( len != 0 )
  {
    ...
    nvReadHdr( findPg, (srcOff - OSAL_NV_HDR_SIZE), (uint8 *)(&hdr) );
    if ( hdr.len < (ndx + len) )
    {
      return NV_OPER_FAILED;
    }
    ...
    if ( cnt != 0 )  // If the buffer to write is different in one or more bytes.
    {
      ...
      if ( dstPg != OSAL_NV_PAGE_NULL )
      {
        ...
        nvReadHdr( dstPg, dstOff, (uint8 *)(&hdr) );
...
uint8 osal_nv_read( uint16 id, uint16 ndx, uint16 len, void *buf )
{
  ...
  nvReadBuf(findPg, offset+ndx, buf, len);
