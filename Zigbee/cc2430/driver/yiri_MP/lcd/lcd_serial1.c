...
static void LCD_GraphicMode_Write(uint8*, uint8, uint8);
/****************************************************************
*** lcd clear
*****************************************************************/
...
{
	P1SEL &= ~0xe0;			// set P1.5, P1.6, P1.7 to general io
	P1DIR |= 0xe0;			// set P1.5, P1.6, P1.7 to output
...
//    P1_0 = 1;
	...
	DelayMs(5);
    LcdWriteReg(0x01, 0x00);    // 开显示,关闭键盘
	DelayMs(5);
	...
	LcdWriteReg(0x10,0xf2);//0xff);	//1/9Bias  最大对比度//0xef
	DelayMs(5);
	LcdWriteReg(0x12,0xfc);		//4x,最小电流驱动//0xfc
	LcdWriteReg(BLTR,0x00);	
    DelayMs(5);
...
#ifndef NEW_MENU_LIB
void LCD_Str_Print(uint8 *pdata, uint8 len, uint8 x, uint8 y, bool direction)
{
 ...
 void LCD_Clear_Inv(void)
{
 ...
 #else
void LCD_Line_Inv(uint8 line)
{
	LcdWriteReg(BLTR,0x00);
	LcdWriteReg(SWSXR,0x00);
	LcdWriteReg(SWSYR,line*LCD_LINE_HIGH);
	LcdWriteReg(SWRXR,0x10);
	LcdWriteReg(SWRYR,0x90);   
	LcdWriteReg(BLTR,0x10);
}
 #endif
...
void LCD_GraphicMode_Write(uint8*p, uint8 x, uint8 y)
{
...
uint8 LCD_Battery_Show(uint8 index)
{
	 ...
	 LCD_GraphicMode_Write((uint8*)p, 14, 0);
	 return LCD_SUCCESS;
}
