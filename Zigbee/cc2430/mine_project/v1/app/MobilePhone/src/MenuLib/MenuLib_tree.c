...
static char*  const __code  ItemList_Settings_Release[] = {RINGSETTING_CHINA,TIMESETTING_CHINA,BACKLIGHTSETTING_CHINA,PADLOCKSETTING_CHINA,RESTOREDEFAULT_CHINA};
...
#endif
static char*  const __code    ItemList_DisParameter[] = {PARAMETER_NEW121220_CHINA,PARAMETER_NEW120907_CHINA,PARAMETER_NEW120104_CHINA,PARAMETER_OLD_CHINA,PARAMETER_DEFAULT_CHINA};
...
static void    menu_call_record_onkey(uint8 keys, uint8 status)
{
    ...
    case HAL_KEY_CALL:
        ...
        {
            osal_nv_read(NV_ID, (node_info.sel_item-1)*sizeof(Record)+1, NMBRDIGIT, dialnum_buf.p);
            dialnum_buf.len = osal_strlen((char*)dialnum_buf.p);
            dialnum_buf.p[dialnum_buf.len] = '\0';
            menu_Dial(dialnum_buf);
        }
...
static void    menu_shortmessage_inbox_display(void)
{
    ...
    else if(node.ChildNum <= SCREEN_LINES)
    {
        ...
        for(i=node.ChildNum, j=1; i>0; i--, j++)
        {
            ...
            tmp = (i-1)*SMS_NV_LEN+SMS_LEN_TYPE_SIZE+1+sizeof(uint16);
#ifdef NEW_DOUBLE_NVID_OP
    ...
    else
    {
        ...
        for(i=0; i<SCREEN_LINES; i++)
        {
            ...
            tmp = (node_info.show_item-i-1)*SMS_NV_LEN+SMS_LEN_TYPE_SIZE+1+sizeof(uint16);
#ifdef NEW_DOUBLE_NVID_OP
            menu_doubleNVID_read(SMS_NVID, tmp, NMBRDIGIT, num_buf.p);
#endif
    ...
    if(input_status == OUTPUT_STATUS)
    {
        ...
        else if(keys == HAL_KEY_SELECT)
        {
            if(data_buf.len > 0)
            {
                ...
                {
                    uint16 offset = (node_info_temp.sel_item-1)*SMS_NV_LEN+SMS_LEN_TYPE_SIZE+1+sizeof(uint16) ;
#ifdef NEW_DOUBLE_NVID_OP
...
static void    menu_shortmessage_read_display(void)
{
    ...
    len_temp = (node_info.sel_item-1)*SMS_NV_LEN + 1+sizeof(uint16);
#ifdef NEW_DOUBLE_NVID_OP
