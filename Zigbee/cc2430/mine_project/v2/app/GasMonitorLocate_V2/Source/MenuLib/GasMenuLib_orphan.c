...
static void    menu_retreat_display(void);
static void    menu_retreat_onkey(uint8 keys, uint8 status);
...
static Orphan_node_t const __code Menu_Orphan[] =
{
    ...
    {
        .ID = MENU_ID_RETREAT,
        .oper.display = menu_retreat_display,
        .oper.on_key = menu_retreat_onkey,
    }    
    ,
    {
        .ID = MENU_ID_MAIN,
...
static void    menu_main_display(void)
{
    ...
    if(ch4_GetLdoStatus() == true) // if Ldo is On
    {
        uint16 density;
        uint8 n1,n2,n3;
        density = (uint16) ch4_get_density();
...
static void    menu_SOS_display(void)
{
    ...
    if(menu_SOSseq <= CALLINGHELP_MAXCNT_LOCATE)
    {
...
void    menu_retreat_display(void)
{

    LcdClearDisplay();
    LCD_Str_Print(RETREAT_CHINA, (LCD_LINE_WIDTH-osal_strlen((char *)RETREAT_CHINA))/2, 1, TRUE);
    LCD_Str_Print(RETREAT_CHINA_EN, (LCD_LINE_WIDTH-osal_strlen((char *)RETREAT_CHINA_EN))/2, 2, TRUE);
